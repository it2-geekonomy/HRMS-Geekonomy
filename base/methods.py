...
def export_data(
    request, model, form_class, filter_class, file_name, perm=None, base_queryset=None
):
    fields_mapping = {
        "male": _("Male"),
        "female": _("Female"),
    ...
    if base_queryset is not None:
        export_objects = filter_class(request.GET, queryset=base_queryset).qs
    else:
...
def is_holiday(date):
    ...
    """
    # For recurring holidays: check if the date matches the month/day of start_date
    # and is within the recurrence period (start_date year <= date year, date <= end_date)
    # For non-recurring holidays: check if the date matches exactly start_date
    holiday = Holidays.objects.filter(
        Q(recurring=True, start_date__month=date.month, start_date__day=date.day, 
          start_date__year__lte=date.year, end_date__gte=date)
        | Q(recurring=False, start_date=date)
    ).first()
    ...
    for company_leave in company_leaves:
        ...
        for month in range(1, 13):
            if based_on_week is not None:
                ...
                week_index = int(based_on_week)
                if week_index >= len(month_calendar):
                    continue
                weeks = month_calendar[week_index]
