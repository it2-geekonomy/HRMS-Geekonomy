...
class DocumentTemplate(HorillaModel):
    """
    Document template for generating employee documents (e.g. Employment Agreement).
    Body uses {{ placeholder }} syntax; placeholders are replaced with employee/company data.
    """

    TEMPLATE_TYPE_CHOICES = [
        ("employment_agreement", _("Employment Agreement")),
    ...
    body = models.TextField(
        ...
        help_text=_("Use {{ variable }} for placeholders. Ignored when an uploaded template file is used."),
        blank=True,
    )
    template_file = models.FileField(
        ...
        null=True,
        blank=True,
        verbose_name=_("Upload Template (Word .docx)"),
        help_text=_(
            "Upload a Word (.docx) file with placeholders like {{ employee_name }}, {{ company_name }}, "
            "{{ job_position }}, {{ agreement_date }}. The generated document will keep your exact layout and formatting."
        ),
    )
    signatory_name = models.CharField(
        max_length=200, blank=True, null=True, verbose_name=_("Signatory Name")
    )
    signatory_designation = models.CharField(
        max_length=200, blank=True, null=True, verbose_name=_("Signatory Designation")
    )
    company_id = models.ForeignKey(
        Company,
        null=True,
...
class SlackConfiguration(HorillaModel):
    """
    SingletonModel to keep Slack API configuration
    """
    ...
    slack_bot_token = models.CharField(
        ...
        null=True,
        blank=True,
        verbose_name=_("Slack Bot Token"),
        help_text=_("Bot User OAuth Token with users:read scope"),
    )

    is_active = models.BooleanField(
        default=True,
        verbose_name=_("Enable Slack Integration"),
    )

    def save(self, *args, **kwargs) -> None:
        # Keep only one active configuration
        if self.is_active:
            SlackConfiguration.objects.filter(is_active=True).exclude(
    ...
    class Meta:
        ...
        verbose_name_plural = _("Slack Configurations")
