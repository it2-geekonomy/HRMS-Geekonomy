...
class AssignPermission(Form):
    ...
    def save(self):
        ...
        Only updates users that are linked to the selected employees
        (employees without employee_user_id are skipped).
        """
        user_ids = list(
        ...
        # Filter out None: employees without a linked login account
...
class DocumentTemplateForm(forms.ModelForm):
    """Form for creating/editing document templates (e.g. Employment Agreement)."""

    class Meta:
        model = DocumentTemplate
        fields = "__all__"
        widgets = {
            "body": forms.Textarea(attrs={"rows": 16, "class": "oh-input w-100"}),
    ...
    def clean(self):
        ...
        template_file = cleaned.get("template_file")
        has_file = template_file or (self.instance and self.instance.pk and self.instance.template_file)
        if not body and not has_file:
            raise forms.ValidationError(
                _("Provide either template content (text) or upload a Word (.docx) template file.")
            )
        return cleaned

    def get_placeholder_help(self):
        """Return dict of placeholder name -> description for UI help."""
        return {
            "agreement_date": _("Date of agreement (e.g. today)"),
            "company_name": _("Company name"),
            "company_address": _("Full company address"),
            "employee_name": _("Employee full name"),
            "employee_address": _("Employee address (with city, state, zip)"),
            "job_position": _("Job position / designation"),
            "employment_commencement_date": _("Joining date"),
            "work_location": _("Office/work location address"),
            "probation_months": _("Probation period in months"),
            "signatory_name": _("Authorized signatory name"),
            "signatory_designation": _("Signatory designation (e.g. Human Resources)"),
        }
