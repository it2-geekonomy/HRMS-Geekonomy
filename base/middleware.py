...
class CompanyMiddleware:
    ...
    def _add_company_filter(self, model, company_id):
        ...
        # Asset allocations: most assets have no batch/lot — filter by assignee company,
        # with fallbacks for asset batch and temporary (non-employee) allocations.
        if model._meta.model_name == "assetassignment" and is_horilla_manager:
            model.add_to_class(
                "company_filter",
                Q(assigned_to_employee_id__employee_work_info__company_id=company_id)
                | Q(asset_id__asset_lot_number_id__company_id=company_id)
                | Q(
                    assigned_to_employee_id__isnull=True,
                    assigned_by_employee_id__employee_work_info__company_id=company_id,
                ),
            )
            return
    ...
    def __call__(self, request):
        if getattr(request, "user", False) and not request.user.is_anonymous:
            ...
            redirect_response = self._set_company_session(request, company_id)
...
class ForcePasswordChangeMiddleware:
    ...
    Set FORCE_PASSWORD_CHANGE_NEW_USERS = True in settings to enable.
    """
    ...
    def __call__(self, request):
        ...
        if hasattr(request, "user") and request.user.is_authenticated:
            ...
            if getattr(settings, "FORCE_PASSWORD_CHANGE_NEW_USERS", False):
