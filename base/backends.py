...
class DefaultHorillaMailBackend(EmailBackend):
    ...
    def get_dynamic_email_config():
        ...
        try:
            ...
            logger.debug("get_dynamic_email_config: could not get company from request: %s", e)
            ...
            if request:
                try:
                    if (
                        ...
                        emp = getattr(request.user, "employee_get", None)
                        if emp is not None:
                            display_email_name = f"{emp.get_full_name()} <{emp.get_email()}>"
                    if request.user.is_authenticated:
                        user_id = request.user.pk
                        emp = getattr(request.user, "employee_get", None)
                        if emp is not None:
                            reply_to = [
                                f"{emp.get_full_name()} <{emp.get_email()}>",
                            ]
                            cache.set(f"reply_to{request.user.pk}", reply_to)
                except Exception as e:
                    logger.debug("get_dynamic_email_config: could not set display/reply from user: %s", e)
...
def _first_recipient_email(recipients):
    """EmailLog.to is a single EmailField; Django's message.to is a list."""
    if not recipients:
        return ""
    first = recipients[0]
...
class ConfiguredEmailBackend(BACKEND_CLASS):
    ...
    def send_messages(self, email_messages):
        ...
        if resend_api_key:
            from base.resend_mail import send_messages_via_resend

            # Always use .env DEFAULT_FROM_EMAIL for Resend (not Mail Server UI).
            self.configuration = self.get_dynamic_email_config()
            default_from = getattr(settings, "DEFAULT_FROM_EMAIL", None)
            try:
                response = send_messages_via_resend(
                    email_messages,
                    default_from_email=default_from,
                    fail_silently=bool(self.dynamic_fail_silently),
                )
            except Exception:
                response = 0
                if not self.dynamic_fail_silently:
                    for message in email_messages:
                        EmailLog(
                            subject=message.subject,
                            from_email=default_from,
                            to=_first_recipient_email(message.to or []),
                            body=message.body,
                            status="failed",
                        ).save()
                    raise
