...
from base.document_generator import (
    ...
    generate_document_from_uploaded_docx,
...
# Minimal valid PNG (1x1) for mail server test when no logo file is available
_MAIL_TEST_PLACEHOLDER_PNG = base64.b64decode(
    "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8"
    "z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg=="
)


def _mail_test_safe_company_icon_path(icon_name):
    if not icon_name:
        return None
    candidate = path.normpath(path.join(settings.MEDIA_ROOT, icon_name))
    media_root = path.normpath(settings.MEDIA_ROOT)
    try:
        if path.commonpath([candidate, media_root]) != media_root:
            return None
    except ValueError:
        return None
...
def _mail_test_logo_bytes_and_subtype():
    """
    Return (bytes, mime_subtype) for the inline test-mail image when no company icon.
    Prefers Geekonomy favicon, then Horilla default; uses staticfiles finder, then
    a tiny embedded PNG.
    """
    rel_parts_options = (
        ("images", "ui", "Geekonomy Favicon Logo.png"),
        ("images", "ui", "horilla-logo.png"),
    )
    for parts in rel_parts_options:
        rel_finder = "/".join(parts)
        static_root_logo = path.join(settings.STATIC_ROOT, *parts)
        for candidate in (static_root_logo, finders.find(rel_finder)):
            if candidate and path.isfile(candidate):
                ext = path.splitext(candidate)[1].lower().lstrip(".")
                subtype = {"jpg": "jpeg", "jpeg": "jpeg"}.get(ext, ext or "png") or "png"
                with open(candidate, "rb") as handle:
                    data = handle.read()
                if data:
                    return data, subtype
    return _MAIL_TEST_PLACEHOLDER_PNG, "png"


def custom404(request, exception=None):
    """
    Custom 404 method. Returns HTTP 404 so logs and clients see correct status.
    """
...
def login_user(request):
    ...
    if request.method == "POST":
        ...
        except User.employee_get.RelatedObjectDoesNotExist:
            employee = None
        if employee is None:
...
def mail_server_test_email(request):
    ...
    company_name = "Geekonomy Technology Private Limited"
    image_bytes = None
    image_mime_subtype = "png"
    ...
    if white_labelling:
        ...
        if company:
            ...
            safe_icon = _mail_test_safe_company_icon_path(icon_name)
            if safe_icon:
                ext = path.splitext(safe_icon)[1].lower().lstrip(".")
                image_mime_subtype = (
                    {"jpg": "jpeg", "jpeg": "jpeg"}.get(ext, ext or "png") or "png"
                )
                with open(safe_icon, "rb") as handle:
                    image_bytes = handle.read()

    if not image_bytes:
        image_bytes, image_mime_subtype = _mail_test_logo_bytes_and_subtype()
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from Geekonomy HRMS")
            ...
            try:
                ...
                msg_img = MIMEImage(image_bytes, _subtype=image_mime_subtype)
...
# ---------- Document Templates (e.g. Employment Agreement) ----------


@login_required
@permission_required("base.view_documenttemplate")
def view_document_templates(request):
    """List document templates (Employment Agreement, etc.)."""
    templates = DocumentTemplate.objects.all()
    form = DocumentTemplateForm()
    placeholder_help = form.get_placeholder_help()
    if templates.exists():
        template_name = "base/document_templates/view_templates.html"
    else:
        template_name = "base/document_templates/empty_document_templates.html"
    return render(
        request,
        template_name,
        {
...
@login_required
@hx_request_required
@permission_required("base.change_documenttemplate")
def view_document_template(request, obj_id):
    """Display/edit a single document template."""
    doc_template = get_object_or_404(DocumentTemplate, id=obj_id)
    form = DocumentTemplateForm(instance=doc_template)
    placeholder_help = form.get_placeholder_help()
    if request.method == "POST":
        form = DocumentTemplateForm(request.POST, request.FILES, instance=doc_template)
        if form.is_valid():
            form.save()
    ...
    return render(
        request,
        "base/document_templates/htmx/form.html",
        {"form": form, "duplicate": False, "placeholder_help": placeholder_help},
    )


@login_required
@hx_request_required
@permission_required("base.add_documenttemplate")
...
@login_required
@permission_required("base.delete_documenttemplate")
def delete_document_templates(request):
    ids = request.GET.getlist("ids")
    DocumentTemplate.objects.filter(id__in=ids).delete()
    messages.success(request, _("Template(s) deleted."))
    return redirect(view_document_templates)
...
def generate_document(request):
    """
    Generate document (PDF or DOCX) for a selected employee and document template.
    GET: show form (employee, template, format). POST: generate and return file.
    """
    ...
    if request.method == "GET" and not templates.exists():
        messages.info(request, _("Create at least one document template first."))
        return redirect("view-document-templates")
    ...
    if request.method == "POST":
        ...
        if not template_id or not employee_id:
            messages.error(request, _("Please select both a template and an employee."))
            return redirect("view-document-templates")
        doc_template = get_object_or_404(DocumentTemplate, id=template_id)
        employee = get_object_or_404(Employee, id=employee_id)
        agreement_date_str = request.POST.get("agreement_date")
        agreement_date = None
        ...
        try:
            # Use uploaded Word template when available (keeps your exact format)
            if doc_template.template_file:
                if fmt == "pdf":
                    messages.error(
                        request,
                        _(
                            "PDF is not available for uploaded templates. "
                            "Please choose Word (DOCX) to keep your template's exact format."
                        ),
                    )
                    return redirect("view-document-templates")
                content = generate_document_from_uploaded_docx(
                    doc_template, employee, agreement_date
                )
                content_type = "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                ext = "docx"
            elif fmt == "docx":
                content = generate_document_docx(doc_template, employee, agreement_date)
                content_type = "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                ext = "docx"
            else:
                content = generate_document_pdf(doc_template, employee, agreement_date)
                content_type = "application/pdf"
                ext = "pdf"
        except ImportError as e:
            messages.error(
                request,
                _("Word export requires python-docx. Install with: pip install python-docx"),
            )
            return redirect("view-document-templates")
        except Exception as e:
            messages.error(request, _("Error generating document: %s") % str(e))
            return redirect("view-document-templates")

        filename = f"{doc_template.name}_{employee.get_full_name().replace(' ', '_')}.{ext}"
    ...
    return render(
        request,
        "base/document_templates/generate_document.html",
        {
            "templates": templates,
            "employees": employees,
        },
...
def update_permission(
    ...
    """
    This method is used to update or clear user permissions.
    Assignments only apply to employees who have a linked user account.
    """
    form = AssignPermission(request.POST)
    if form.is_valid():
        employee_ids = form.data.getlist("employee")
        employees = list(Employee.objects.filter(id__in=employee_ids))
        employees_without_user = [e for e in employees if e.employee_user_id_id is None]
        permissions_list = form.cleaned_data.get("permissions") or []
        # If assigning permissions and no selected employee has a linked user, show error
        if permissions_list and len(employees_without_user) == len(employees):
            names = ", ".join(e.get_full_name() for e in employees_without_user[:3])
            if len(employees_without_user) > 3:
                names += _(" and %(count)s more") % {"count": len(employees_without_user) - 3}
            return JsonResponse(
                {
                    "message": _(
                        "No login account linked for: %(names)s. "
                        "Link a user account in the employee profile (About tab) first."
                    )
                    % {"names": names},
                    "type": "danger",
                }
        ...
        form.save()
        if not permissions_list and employee_ids:
            return JsonResponse({"message": _("All permissions cleared."), "type": "info"})
        return JsonResponse({"message": _("Updated the permissions."), "type": "success"})
    # Only clear permissions when form is invalid due to empty permissions (explicit clear)
    employee_ids = form.data.getlist("employee")
    if employee_ids and not form.data.getlist("permissions"):
        emp = Employee.objects.filter(id=employee_ids[0]).first()
        if emp and emp.employee_user_id_id is not None:
            emp.employee_user_id.user_permissions.clear()
            return JsonResponse({"message": _("All permissions cleared."), "type": "info"})
    return JsonResponse({"message": _("Something went wrong."), "type": "danger"})
...
def check_chart_permission(request, charts):
    ...
    chart_apps = {
        ...
        "notification_chart": "notifications",
        "my_key_result_status": "pms",
        "my_leave_summary": "leave",
        "my_hours_summary": "attendance",
    }
    permissions = {
...
def employee_chart_show(request):
    ...
    charts = [
        ...
        # ("attendance_analytic", _("Attendance analytics")),  # Commented out - not needed for now
        # ("hours_chart", _("Hours Chart")),  # Commented out - not needed for now
        # ("employees_chart", _("Employees Chart")),  # Commented out - not needed for now
        ("department_chart", _("Department Chart")),
        # ("gender_chart", _("Gender Chart")),  # Commented out - not needed for now
        # ("objective_status", _("Objective Status")),  # Commented out - not needed for now
        ...
        ("notification_chart", _("My Notifications")),
        ("my_key_result_status", _("My Key Results")),
        ("my_leave_summary", _("My Leave Summary")),
        ("my_hours_summary", _("My Hours")),
    ]
...
@login_required
def my_key_result_status(request):
    """
    Employee's own key result status for dashboard - returns JSON for chart
    """
    from django.apps import apps
    ...
    if not apps.is_installed("pms"):
        if is_ajax:
            return JsonResponse({"message": _("PMS not installed")})
        return render(request, "base/dashboard/my_key_result_status.html", {"data": None})
    ...
    try:
        ...
        if is_ajax:
            return JsonResponse({"message": _("No employee profile")})
        return render(request, "base/dashboard/my_key_result_status.html", {"data": None})
    
    # Get key results for this employee
    key_results = EmployeeKeyResult.objects.filter(
        employee_objective_id__employee_id=employee
    )
    
    if is_ajax:
        data = {"message": _("No key results found")}
        for status_choice in EmployeeKeyResult.STATUS_CHOICES:
            count = key_results.filter(status=status_choice[0]).count()
            if count > 0:
                data.setdefault("labels", []).append(str(status_choice[1]))
                data.setdefault("values", []).append(count)
        return JsonResponse(data)
    
    # For non-ajax, return template
    status_data = {}
    for status_choice in EmployeeKeyResult.STATUS_CHOICES:
        count = key_results.filter(status=status_choice[0]).count()
        if count > 0:
            status_data[status_choice[1]] = count
    
    context = {
        'data': status_data,
        'total': key_results.count(),
    }
    return render(request, "base/dashboard/my_key_result_status.html", context)


@login_required
def my_leave_summary(request):
    """
    Employee's own leave summary for dashboard - returns JSON for chart
    """
    from django.apps import apps
    from django.http import JsonResponse
    ...
    if not apps.is_installed("leave"):
        if is_ajax:
            return JsonResponse({"message": _("Leave not installed")})
        return render(request, "base/dashboard/my_leave_summary.html", {"data": None})
    ...
    # Get leave requests for this employee this year
    ...
    if is_ajax:
        # Count by leave type for approved leaves
        leave_types = {}
        for lr in leave_requests.filter(status='approved'):
            lt_name = lr.leave_type_id.name if lr.leave_type_id else "Other"
            leave_types[lt_name] = leave_types.get(lt_name, 0) + 1
        ...
        if leave_types:
            ...
            data = {"message": _("No approved leaves this year")}
        
        # Also add status counts
        data["pending"] = leave_requests.filter(status='requested').count()
        data["approved"] = leave_requests.filter(status='approved').count()
        data["rejected"] = leave_requests.filter(status='rejected').count()
        
        return JsonResponse(data)
    
    # For non-ajax
    pending = leave_requests.filter(status='requested').count()
    ...
    return render(request, "base/dashboard/my_leave_summary.html", context)


@login_required
def my_hours_summary(request):
    """
    Employee's own hours summary for dashboard - last 4 weeks worked hours + total working days.
    Returns JSON for chart (labels = week ranges, worked_values = hours per week).
    """
    ...
    if not apps.is_installed("attendance"):
        if is_ajax:
            return JsonResponse({"message": _("Attendance not installed")})
        return render(request, "base/dashboard/my_hours_summary.html", {"data": None})
    ...
    # Last 4 weeks: week 0 = most recent 7 days, week 1 = 7–14 days ago, etc.
    for i in range(4):
        ...
        for att in attendances:
            week_seconds += (att.at_work_second or 0)
        total_worked_seconds += week_seconds
        worked_values.append(round(week_seconds / 3600, 1))

    labels.reverse()
    worked_values.reverse()
    # Total working days = distinct days with attendance in last 28 days
    from_date = today - timedelta(days=27)
    total_working_days = Attendance.objects.filter(
        employee_id=employee,
        attendance_date__gte=from_date,
        attendance_date__lte=today,
    ).values("attendance_date").distinct().count()
    total_worked_hours = round(total_worked_seconds / 3600, 1)
    avg_hours = round(total_worked_hours / total_working_days, 1) if total_working_days > 0 else 0

    if is_ajax:
        data = {
            "labels": labels,
            "worked_values": worked_values,
            "total_working_days": total_working_days,
            "total_worked_hours": total_worked_hours,
            "avg_hours": avg_hours,
            "message": _("No attendance in last 4 weeks") if total_working_days == 0 else "",
        }
        return JsonResponse(data)
...
@login_required
@permission_required("base.change_slackconfiguration")
def slack_configuration(request):
    """
    View to manage Slack API configuration and employee-Slack user ID mappings
    """
    ...
    slack_config = SlackConfiguration.objects.first()

    class SlackConfigForm(forms.ModelForm):
        class Meta:
            model = SlackConfiguration
            fields = ["slack_bot_token", "is_active"]

    class SlackMappingForm(forms.Form):
        employee_id = forms.IntegerField(widget=forms.HiddenInput())
        slack_user_id = forms.CharField(
            max_length=50,
            required=False,
            label="Slack User ID",
            help_text="Slack user ID (e.g., U01234ABCD)"
        )

    if request.method == "POST":
        if "save_config" in request.POST:
            form = SlackConfigForm(request.POST, instance=slack_config)
            if form.is_valid():
                form.save()
                messages.success(request, _("Slack configuration saved successfully"))
                return HttpResponseRedirect(request.META.get("HTTP_REFERER", "/"))
        elif "save_mappings" in request.POST:
            employee_id = request.POST.get("employee_id")
            slack_user_id = request.POST.get("slack_user_id")
            if employee_id:
                employee = Employee.objects.filter(id=employee_id).first()
                if employee:
                    employee.slack_user_id = slack_user_id if slack_user_id else None
                    employee.save()
                    messages.success(request, _("Employee Slack mapping updated successfully"))
                else:
                    messages.error(request, _("Employee not found"))
            return HttpResponseRedirect(request.META.get("HTTP_REFERER", "/"))

    config_form = SlackConfigForm(instance=slack_config)
