...
def live_unread_notification_list(request):
    """Return a json with a unread notification list. Cached 30s server-side to reduce load."""
    try:
        user_is_authenticated = request.user.is_authenticated()
    except TypeError:  # Django >= 1.11
        user_is_authenticated = request.user.is_authenticated
...
@login_required
def dashboard_notification_chart(request):
    """
    Dashboard view to show notification statistics and list
    """
    ...
    all_notifications = Notification.objects.filter(recipient=user, deleted=False)
    
    # Count by read/unread status
    unread_count = all_notifications.filter(unread=True).count()
    read_count = all_notifications.filter(unread=False).count()
    
    # Get recent notifications (last 10) for display
    recent_notifications = all_notifications.order_by('-timestamp')[:10]
    ...
    return render(request, "notifications/dashboard_notification_chart.html", context)
