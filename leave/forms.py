...
from leave.methods import get_init_days_and_reset_for_assign, get_leave_day_attendance
...
# Leave types that allow only Full Day (no First Half / Second Half).
...
class LeaveRequestCreationForm(BaseModelForm):
    ...
    def clean(self):
        ...
        if leave_type and leave_type.name in FULL_DAY_ONLY_LEAVE_TYPES:
            cleaned_data["start_date_breakdown"] = "full_day"
            cleaned_data["end_date_breakdown"] = "full_day"
        return cleaned_data
...
class LeaveRequestUpdationForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Probation Leave / Interns Leave: only Full Day allowed
        if leave_type and leave_type.name in FULL_DAY_ONLY_LEAVE_TYPES:
            full_day_only = [
                c for c in LeaveRequest._meta.get_field("start_date_breakdown").choices if c[0] == "full_day"
            ]
            self.fields["start_date_breakdown"].choices = full_day_only
            self.fields["end_date_breakdown"].choices = full_day_only

    def clean(self):
        cleaned_data = super().clean()
        leave_type = cleaned_data.get("leave_type_id") or (
            self.instance and getattr(self.instance, "leave_type_id", None)
        )
        if leave_type and leave_type.name in FULL_DAY_ONLY_LEAVE_TYPES:
            cleaned_data["start_date_breakdown"] = "full_day"
            cleaned_data["end_date_breakdown"] = "full_day"
...
class UserLeaveRequestForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["attachment"].widget.attrs["accept"] = ".jpg, .jpeg, .png, .pdf"
        if employee:
            available_leaves = employee.available_leave.exclude(
            ...
            self.fields["leave_type_id"].queryset = assigned_leave_types
        if leave_type and isinstance(leave_type, dict):
            lt = leave_type.get("leave_type_id")
            if lt is not None:
                self.fields["leave_type_id"].queryset = LeaveType.objects.filter(id=lt.id)
                self.fields["leave_type_id"].initial = lt.id
                self.fields["leave_type_id"].empty_label = None
                # Probation Leave / Interns Leave etc.: no breakdown fields shown, always full day
                if lt.name in FULL_DAY_ONLY_LEAVE_TYPES:
                    self.fields["start_date_breakdown"].widget = forms.HiddenInput()
                    self.fields["end_date_breakdown"].widget = forms.HiddenInput()
                    self.initial["start_date_breakdown"] = "full_day"
                    self.initial["end_date_breakdown"] = "full_day"
...
class UserLeaveRequestCreationForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if employee:
            ...
            self.fields["leave_type_id"].queryset = assigned_leave_types
            if assigned_leave_types.filter(name="Sick Leave").exists():
                self.fields["attachment"].help_text = _(
...
class CompOffRequestForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["start_date_breakdown"].label = _("Breakdown")

    def clean(self):
        cleaned_data = super().clean()
        start = cleaned_data.get("start_date")
        end = cleaned_data.get("end_date")
        start_bd = cleaned_data.get("start_date_breakdown")
        end_bd = cleaned_data.get("end_date_breakdown")
        if start and end and start == end:
            cleaned_data["end_date_breakdown"] = start_bd
        elif start_bd and not end_bd:
            cleaned_data["end_date_breakdown"] = start_bd
        return cleaned_data
    ...
    class Meta:
        ...
        widgets = {
            "end_date_breakdown": forms.HiddenInput(),
            ...
            "start_date_breakdown": forms.Select(attrs={"class": "oh-select w-100"}),
        }


class CompOffRequestApproveForm(forms.Form):
    approved_days = forms.FloatField(
        label=_("Comp-Off Days to Assign"),
        min_value=0.5,
        widget=forms.NumberInput(attrs={"class": "oh-input w-100", "step": "0.5"}),
    )


class CompOffRequestRejectForm(forms.Form):
    reason = forms.CharField(
        label=_("Rejection Reason"),
        widget=forms.Textarea(attrs={"rows": 4, "class": "p-4 oh-input w-100"}),
    )
