...
from .models import LeaveAllocationRequest, CompOffRequest
...
@decorator_with_arguments
def comp_off_approve_permission(function=None, *args, **kwargs):
    """Only the employee's reporting manager may approve or reject."""

    def check_permission(request, req_id=None, *args, **kwargs):
        try:
            comp_off_request = CompOffRequest.objects.get(id=req_id)
            reporting_manager = comp_off_request.reporting_manager()
            if reporting_manager and request.user.employee_get == reporting_manager:
                return function(request, *args, req_id=req_id, **kwargs)
            messages.info(request, _("Only the reporting manager can approve this request."))
            previous_url = request.META.get("HTTP_REFERER", "/")
            script = f'<script>window.location.href = "{previous_url}"</script>'
            if "HTTP_HX_REQUEST" in request.META:
                return render(request, "decorator_404.html")
            return HttpResponse(script)
        except (CompOffRequest.DoesNotExist, OverflowError, ValueError):
            messages.error(request, _("Comp-Off request not found"))
            return redirect(reverse("comp-off-request-view"))

    return check_permission


@decorator_with_arguments
def comp_off_change_permission(function=None, *args, **kwargs):
    def check_permission(request, req_id=None, *args, **kwargs):
        try:
            comp_off_request = CompOffRequest.objects.get(id=req_id)
            if (
                request.user.has_perm("leave.change_compoffrequest")
                or request.user.employee_get == comp_off_request.employee_id
                or request.user.employee_get == comp_off_request.reporting_manager()
            ):
                return function(request, *args, req_id=req_id, **kwargs)
            messages.info(request, _("You dont have permission."))
            previous_url = request.META.get("HTTP_REFERER", "/")
