...
SUBMENUS = [
    ...
    {
        "menu": trans("My Comp-Off Requests"),
        "redirect": reverse("my-comp-off-request-view"),
    },
    {
    ...
    {
        "menu": trans("Comp-Off Requests"),
        "redirect": reverse("comp-off-request-view"),
        "accessibility": "leave.sidebar.comp_off_request_accessibility",
    },
    {
    ...
    # Assigned Leave moved to Configuration in sidebar.html
    # {
    ...
    # Restrict Leaves not used in our company - hidden for all users (admin/staff)
    # {
...
def company_leave_accessibility(request, submenu, user_perms, *args, **kwargs):
    """Show Company Leaves only to staff or users with view permission; hide from employees."""
    return (
        request.user.is_staff
        or request.user.has_perm("base.view_companyleaves")
    )


def restrict_leave_accessibility(request, submenu, user_perms, *args, **kwargs):
    """Show Restrict Leaves only to staff or users with view permission; hide from employees."""
    return (
        request.user.is_staff
        or request.user.has_perm("leave.view_restrictleave")
    )
