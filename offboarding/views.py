...
def _move_stage(stage_id, direction):
    """
    Swap sequence of the stage with previous (up) or next (down) stage.
    direction: 'up' or 'down'
    Returns (stage, True) if moved, (stage, False) if no swap (first/last).
    """
    stage = OffboardingStage.objects.get(id=stage_id)
    siblings = list(
        OffboardingStage.objects.filter(offboarding_id=stage.offboarding_id).order_by(
            "sequence", "id"
        )
    )
    idx = next((i for i, s in enumerate(siblings) if s.id == stage.id), None)
    if idx is None:
        return stage, False
    if direction == "up" and idx > 0:
        ...
        return stage, False
    stage.sequence, other.sequence = other.sequence, stage.sequence
...
def move_stage_up(request):
    """
    Move offboarding stage up (earlier in list). Refreshes pipeline body.
    """
    stage_id = request.GET.get("stage_id") or request.POST.get("stage_id")
    if not stage_id:
        return HttpResponse(status=400)
    stage, moved = _move_stage(stage_id, "up")
    groups = pipeline_grouper(request.GET, [stage.offboarding_id])
    for item in groups:
        setattr(item["offboarding"], "stages", item["stages"])
    stage_forms = {
        str(stage.offboarding_id.id): StageSelectForm(offboarding=stage.offboarding_id)
    }
    pd = request.GET.urlencode()
    return render(
        request,
        "offboarding/stage/stages.html",
        {"offboarding": groups[0], "pd": pd, "stage_forms": stage_forms},
    )
...
@hx_request_required
@offboarding_manager_can_enter("offboarding.change_offboardingstage")
def move_stage_down(request):
    """
    Move offboarding stage down (later in list). Refreshes pipeline body.
    """
    stage_id = request.GET.get("stage_id") or request.POST.get("stage_id")
    if not stage_id:
        return HttpResponse(status=400)
    stage, moved = _move_stage(stage_id, "down")
    groups = pipeline_grouper(request.GET, [stage.offboarding_id])
    for item in groups:
        setattr(item["offboarding"], "stages", item["stages"])
    stage_forms = {
...
@hx_request_required
@offboarding_manager_can_enter("offboarding.change_offboardingstage")
def reorder_stage(request):
    """
    Reorder a stage by dropping it at a new index (0-based).
    POST: stage_id, to_index. Returns pipeline body fragment.
    """
    ...
    from_idx = next((i for i, s in enumerate(siblings) if s.id == stage.id), None)
    stage_forms = {}
    stage_forms[str(stage.offboarding_id.id)] = StageSelectForm(
        offboarding=stage.offboarding_id
    )
    if from_idx is None or from_idx == to_index:
        groups = pipeline_grouper(request.GET, [stage.offboarding_id])
        for item in groups:
            setattr(item["offboarding"], "stages", item["stages"])
        pd = request.GET.urlencode()
    ...
    to_index = min(to_index, len(siblings) - 1)
    siblings.pop(from_idx)
    siblings.insert(to_index, stage)
    for seq, s in enumerate(siblings):
        if s.sequence != seq:
            s.sequence = seq
