...
$("#exportLatecome").click(function (e) {
    ...
    getCurrentLanguageCode(function (code) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var pd = $("#lateEarlyActiveFilters").attr("data-pd");
                var params = pd || $("#filterForm").serialize();
                if (attendanceIds.length) {
                    params +=
                        (params ? "&" : "") +
                        "attendance_ids=" +
                        encodeURIComponent(JSON.stringify(attendanceIds));
                }
                $.ajax({
                    type: "GET",
                    url: "/attendance/late-come-early-out-info-export?" + params,
                    dataType: "binary",
                    xhrFields: {
                        responseType: "blob",
                    },
                    success: function (response) {
                        ...
                        const link = document.createElement("a");
                        link.href = url;
                        link.download = "Late_Early_" + currentDate + ".xlsx";
                        document.body.appendChild(link);
                        link.click();
                    },
                    error: function (xhr, textStatus, errorThrown) {
