...
class Attendance(HorillaModel):
    ...
    def requested_fields(self):
        ...
        if self.requested_data is not None:
            data = parse_attendance_requested_data(self.requested_data)
    ...
    def update_ot(self, employee_ot):
        ...
        for attendance in month_attendances:
            # Handle None values for minimum_hour and at_work_second
            minimum_hour_value = attendance["minimum_hour"]
            at_work_second_value = attendance["at_work_second"]
            
            if minimum_hour_value is None:
                required_work_second = 0
            else:
                required_work_second = strtime_seconds(minimum_hour_value)
            
            if at_work_second_value is None:
                at_work_second_value = 0
            
            at_work_second = min(required_work_second, at_work_second_value)
...
class AttendanceRequestLog(HorillaModel):
    """
    Log of attendance request actions: requested, approved, rejected, or edited.
    Used to show "Attendance Log" in employee profile (who requested, who approved/rejected/edited, when).
    """
    ...
    description = models.TextField(
        ...
        help_text=_("Optional context, e.g. attendance date or request type"),
    )
    employee_request_note = models.TextField(
        blank=True,
        verbose_name=_("Employee request note"),
        help_text=_("Reason or description the employee entered (snapshotted on approval)."),
    )
    requested_data_snapshot = models.JSONField(
        null=True,
        blank=True,
        verbose_name=_("Requested data snapshot"),
        help_text=_("Snapshot of Attendance.requested_data at the time of this action (requested/edited/approved/rejected)."),
    )
    attendance_snapshot = models.JSONField(
        null=True,
        blank=True,
        verbose_name=_("Attendance snapshot"),
        help_text=_("Snapshot of Attendance.serialize() at the time of this action (e.g. approved values)."),
    )

    class Meta:
        ordering = ["-performed_at"]
        verbose_name = _("Attendance request log")
        verbose_name_plural = _("Attendance request logs")

    def __str__(self):
        return f"{self.employee_id} | {self.get_action_display()} | {self.performed_at}"
...
class WorkRecords(models.Model):
    ...
    def title_message(self):
        title_message = self.message or ""
        # Leave app sets message to "On leave"; show leave type (SL, EL, CL, etc.) when available
        if self.is_leave_record and apps.is_installed("leave"):
            leave_req = getattr(self, "leave_request_id", None)
            if leave_req and leave_req.leave_type_id:
                return f"Leave | {leave_req.leave_type_id}"
        return title_message or None
