...
class AttendanceUpdateForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        if instance := kwargs.get("instance"):
            ...
            # Get shift from instance, or fallback to employee's assigned shift
            shift_value = instance.shift_id
            work_type_value = instance.work_type_id
            if shift_value is None and instance.employee_id:
                # Try to get shift from employee's work info
                employee_work_info = getattr(instance.employee_id, 'employee_work_info', None)
                if employee_work_info:
                    shift_value = getattr(employee_work_info, 'shift_id', None)
                    if work_type_value is None:
                        work_type_value = getattr(employee_work_info, 'work_type_id', None)
            ...
            initial = {
                ...
                # Include shift_id and work_type_id from instance or employee's work info
                "shift_id": shift_value,
                "work_type_id": work_type_value,
...
class AttendanceRequestForm(BaseModelForm):
    """
    AttendanceRequestForm — full fields for manager edit/validate flows.
    Use simplified_update=True for employee “change request” (same fields as new request UI).
    """
    ...
    def __init__(self, *args, **kwargs):
        simplified_update = kwargs.pop("simplified_update", False)
        self._simplified_update = simplified_update
        if instance := kwargs.get("instance"):
            # django forms not showing value inside the date, time html element.
            # so here overriding default forms instance method to set initial value
            initial = {
        ...
        if simplified_update:
            # Keep every Meta field on the form so ModelForm._post_clean / construct_instance
            # can resolve attendance_worked_hour, shift_id, etc. Hide extras from the UI.
            visible_order = [
                "attendance_date",
                "attendance_clock_in",
                "attendance_clock_out",
            ...
            for k in visible_order:
                if k in self.fields:
                    new_fields[k] = self.fields[k]
            for k in hidden_rest:
                if k in self.fields:
                    fld = self.fields[k]
            ...
            for fname in ("attendance_date", "attendance_clock_in", "attendance_clock_out"):
                if fname not in self.fields:
                    continue
                attrs = self.fields[fname].widget.attrs
                for hk in list(attrs.keys()):
                    if hk.startswith("hx-"):
                        del attrs[hk]
    ...
    def as_p(self, *args, **kwargs):
        ...
        """
        _ = args, kwargs  # Explicitly mark as used for pylint
        if getattr(self, "_simplified_update", False):
            return render_to_string(
                "requests/attendance/attendance_request_simplified_fields.html",
                {"form": self},
            )
        context = {"form": self}
    ...
    def clean(self) -> Dict[str, Any]:
        super().clean()
        if not getattr(self, "_simplified_update", False):
            return self.cleaned_data
        inst = self.instance
        if not inst or not inst.pk:
            return self.cleaned_data
        ad = self.cleaned_data.get("attendance_date")
        if not ad:
            return self.cleaned_data
        self.cleaned_data["attendance_clock_in_date"] = ad
        self.cleaned_data["attendance_clock_out_date"] = ad
        self.cleaned_data["shift_id"] = inst.shift_id
        self.cleaned_data["work_type_id"] = inst.work_type_id
        self.cleaned_data["minimum_hour"] = inst.minimum_hour or "08:00"
        self.cleaned_data["batch_attendance_id"] = inst.batch_attendance_id

        cin_time = self.cleaned_data.get("attendance_clock_in")
        cout_time = self.cleaned_data.get("attendance_clock_out")
        attendance_worked_hour = "00:00"
        if cin_time and cout_time:
            try:
                date_str = ad.strftime("%Y-%m-%d") if hasattr(ad, "strftime") else str(ad)
                time_in_str = (
                    cin_time.strftime("%H:%M")
                    if hasattr(cin_time, "strftime")
                    else str(cin_time)[:5]
                )
                time_out_str = (
                    cout_time.strftime("%H:%M")
                    if hasattr(cout_time, "strftime")
                    else str(cout_time)[:5]
                )
                cin_dt = parse_datetime(date_str, time_in_str)
                cout_dt = parse_datetime(date_str, time_out_str)
                if cin_dt and cout_dt:
                    total_seconds = int(max(0, (cout_dt - cin_dt).total_seconds()))
                    attendance_worked_hour = format_time(total_seconds)
            except (ValueError, TypeError, AttributeError):
                attendance_worked_hour = "00:00"
        self.cleaned_data["attendance_worked_hour"] = attendance_worked_hour
        return self.cleaned_data
...
class NewRequestForm(AttendanceRequestForm):
    ...
    When initial["_employee_form"] is True, shows simplified fields for employee:
    no Create Bulk, Shift, Work Type, Minimum hour, Worked Hours, Batch Attendance;
    order: Check-In Date then Check-In, Check-Out Date then Check-Out.
    """

    @property
    def is_employee_form(self):
        """Template-safe flag (Django disallows attributes starting with _)."""
        return getattr(self, "_employee_form", False)

    def __init__(self, *args, **kwargs):
        view_initial = kwargs.get("initial") or {}
        self._employee_form = view_initial.pop("_employee_form", False)
        ...
        new_dict = OrderedDict([
            (
                ...
                forms.ModelChoiceField(
                    ...
                    if not self._employee_form
                    else forms.Select(attrs={"class": "oh-select oh-select-2 w-100"}),
        ...
        # Check-In/Check-Out Date hidden in UI; we set them from attendance_date in clean().
        self.fields["attendance_clock_in_date"].required = False
        self.fields["attendance_clock_out_date"].required = False
        if self._employee_form:
            # Employee form: only these fields; Check-In/Check-Out Date hidden (use attendance_date).
            ...
            self.fields = OrderedDict(
                (k, self.fields[k]) for k in employee_field_order if k in self.fields
            )
    ...
    def clean(self) -> Dict[str, Any]:
        ...
        if not attendance_date:
            return
        # Use attendance date for check-in/check-out dates (same day); hide those fields in UI.
        ...
        # Employee form does not show shift/work type/min hours/worked hours/batch; use defaults
        if getattr(self, "_employee_form", False):
            get_shift = getattr(employee, "get_shift", None) if employee else None
            shift_id = get_shift() if callable(get_shift) else None
            work_type_id = None
            minimum_hour = "08:00"
            batch_attendance_id = None
            # Compute worked hours from clock-in and clock-out so status is P (not MP) after save
            cin_date = self.cleaned_data.get("attendance_clock_in_date")
            cin_time = self.cleaned_data.get("attendance_clock_in")
            cout_date = self.cleaned_data.get("attendance_clock_out_date")
            cout_time = self.cleaned_data.get("attendance_clock_out")
            if cin_date and cin_time and cout_date and cout_time:
                try:
                    date_str = cin_date.strftime("%Y-%m-%d") if hasattr(cin_date, "strftime") else str(cin_date)
                    time_in_str = cin_time.strftime("%H:%M") if hasattr(cin_time, "strftime") else str(cin_time)[:5]
                    time_out_str = cout_time.strftime("%H:%M") if hasattr(cout_time, "strftime") else str(cout_time)[:5]
                    cin_dt = parse_datetime(date_str, time_in_str)
                    cout_dt = parse_datetime(
                        cout_date.strftime("%Y-%m-%d") if hasattr(cout_date, "strftime") else str(cout_date),
                        time_out_str,
                    )
                    if cin_dt and cout_dt:
                        total_seconds = (cout_dt - cin_dt).total_seconds()
                        attendance_worked_hour = format_time(int(max(0, total_seconds)))
                    else:
                        attendance_worked_hour = "00:00"
                except (ValueError, TypeError, AttributeError):
                    attendance_worked_hour = "00:00"
            ...
            minimum_hour = self.data.get("minimum_hour", "00:00")
            batch_attendance_id = self.cleaned_data.get("batch_attendance_id")
        data = {
        ...
        if attendances.exists():
            ...
            # Do not allow creating/raising another request for the same date if one is already pending
            if attendance.is_validate_request:
                raise ValidationError(
                    _(
                        "A pending attendance request already exists for this date. "
                    )
                )
            data["employee_id"] = employee.id
            data["attendance_date"] = str(attendance_date)
            data["attendance_clock_in_date"] = self.cleaned_data["attendance_clock_in_date"]
            ...
            data["attendance_clock_out_date"] = self.cleaned_data["attendance_clock_out_date"]
            data["work_type_id"] = self.data.get("work_type_id") or (str(work_type_id.id) if work_type_id else "")
            data["shift_id"] = self.data.get("shift_id") or (str(shift_id.id) if shift_id else "")
            ...
            self.updated_attendance = attendance  # so view can send emails for this request
            return

        if getattr(self, "_employee_form", False):
            data["batch_attendance_id"] = None
        ...
        self.updated_attendance = None  # only set when we updated an existing record
        return
...
class BulkAttendanceRequestForm(BaseModelForm):
    ...
    def save(self, commit=True):
        ...
        # Get request user for notifications
        request = getattr(horilla_middlewares._thread_locals, "request", None)
        actor = None
        if request and hasattr(request.user, "employee_get"):
        ...
        for date in date_list:
            ...
            if form.is_valid():
                ...
                if actor and instance.employee_id.employee_work_info.reporting_manager_id:
                    reporting_manager = (
                        instance.employee_id.employee_work_info.reporting_manager_id.employee_user_id
                    )
                    user_last_name = get_employee_last_name(instance)
                    notify_kw = dict(
                        verb=f"{instance.employee_id.employee_first_name} {user_last_name}'s attendance request for {instance.attendance_date} is created",
                        verb_ar=f"تم إنشاء طلب الحضور لـ {instance.employee_id.employee_first_name} {user_last_name} في {instance.attendance_date}",
                        verb_de=f"Die Anwesenheitsanfrage von {instance.employee_id.employee_first_name} {user_last_name} für den {instance.attendance_date} wurde erstellt",
                        verb_es=f"Se ha creado la solicitud de asistencia de {instance.employee_id.employee_first_name} {user_last_name} para el {instance.attendance_date}",
                        verb_fr=f"La demande de présence de {instance.employee_id.employee_first_name} {user_last_name} pour le {instance.attendance_date} a été créée",
                        redirect=reverse("request-attendance-view") + f"?id={instance.id}",
                        icon="checkmark-circle-outline",
                    )
                    if request and actor == request.user:
