...
@register.filter(name="late_punch_flag")
def late_punch_flag(clock_in):
    from attendance.late_punch import punch_flag_for_clock_in

    return punch_flag_for_clock_in(clock_in)
...
@register.filter
def dict_get(d, key):
    """Return d[key] or None if d is None or key not in d. Used for dict lookups in templates."""
    if d is None:
        return None
    try:
        return d.get(key)
...
@register.filter
def hp_p_key(date_val, employee_id):
    """
    Build a string key for (employee_id, date) for HP/P calendar lookup.
    Usage: {% if day|hp_p_key:employee.id in hp_p_dates %}
    """
    if date_val is None or employee_id is None:
        return ""
    if hasattr(date_val, "isoformat"):
        return f"{employee_id}_{date_val.isoformat()}"
    return f"{employee_id}_{date_val}"
