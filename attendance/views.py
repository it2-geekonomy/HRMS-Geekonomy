...
def late_come(attendance, start_time, end_time, shift=None):
    """
    ...
    if shift and shift.grace_time_id:
        ...
        if (
            ...
            # Get grace time allowance for check in
            grace_time_seconds = shift.grace_time_id.allowed_time_in_secs
    # checking default grace time
        ...
        # Get grace time allowance for check in if grace allocate for clock in event
        if grace_time.allowed_clock_in:
            grace_time_seconds = grace_time.allowed_time_in_secs
    
    # Apply grace time to start time - add grace time to start time
    effective_start_time = start_time + grace_time_seconds
    ...
    if start_time > end_time and start_time != end_time:
        ...
        if now_sec < mid_day_sec:
            ...
            if now_sec > effective_start_time:
                late_come_create(attendance)
        elif now_sec > effective_start_time:
            # Here  attendance or attendance activity for previous day night shift
            late_come_create(attendance)
    elif effective_start_time < now_sec:
        late_come_create(attendance)
    return True
...
def early_out(attendance, start_time, end_time, shift=None):
    """
    ...
    # CRITICAL FIX: Don't process early out if employee hasn't checked out yet
    ...
    if shift and shift.grace_time_id:
        ...
        if grace_time.allowed_clock_out:
            grace_time_seconds = grace_time.allowed_time_in_secs
    # Apply grace time to the comparison - subtract grace time from end_time
    effective_end_time = end_time - grace_time_seconds
    
    if start_time > end_time:
        # Early out condition for night shift
        if now_sec < mid_day_sec:
            if now_sec < effective_end_time:
                # Early out condition for general shift
                early_out_create(attendance)
        else:
            early_out_create(attendance)
        return
    if effective_end_time > now_sec:
        early_out_create(attendance)
    return
