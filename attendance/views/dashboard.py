...
def dashboard(request):
    ...
    early_out = find_early_out(start_date=today, end_date=today)
    early_out_count = len(early_out)
...
@login_required
@hx_request_required
def dashboard_card_modal_content(request, card_type):
    """
    Returns HTML fragment for dashboard card modal: on_time, late_come, or early_out.
    Shows today's list for the given card type.
    """
    today = datetime.today().date()
    context = {"card_type": card_type, "date": today}
    if card_type == "on_time":
        attendances = Attendance.objects.filter(attendance_date=today)
        attendances = filtersubordinates(
            request, attendances, perm="attendance.view_attendance"
        )
        late_come_ids = AttendanceLateComeEarlyOut.objects.filter(
            attendance_id__attendance_date=today,
            type="late_come",
        ).values_list("attendance_id_id", flat=True)
        attendances = attendances.exclude(id__in=late_come_ids)
        context["attendances"] = attendances
        return render(
            request,
            "attendance/dashboard/dashboard_card_modal_on_time.html",
            context,
        )
    if card_type == "late_come":
        late_come_qs = find_late_come(start_date=today, end_date=today)
        late_come_qs = filtersubordinates(
            request,
            late_come_qs,
            perm="attendance.view_attendancelatecomeearlyout",
        )
        context["records"] = late_come_qs
        return render(
            request,
            "attendance/dashboard/dashboard_card_modal_late_come_early_out.html",
            context,
        )
    if card_type == "early_out":
        early_out_qs = find_early_out(start_date=today, end_date=today)
        early_out_qs = filtersubordinates(
            request,
            early_out_qs,
            perm="attendance.view_attendancelatecomeearlyout",
        )
        context["records"] = early_out_qs
        return render(
            request,
            "attendance/dashboard/dashboard_card_modal_late_come_early_out.html",
            context,
        )
    return render(request, "attendance/dashboard/dashboard_card_modal_empty.html", context)
