...
def late_come_early_out_search(request):
    ...
    if field != "" and field is not None:
        ...
        reports = LateComeEarlyOutFilter(get_params).qs
        ...
        reports = group_by_queryset(reports, field, get_params.get("page"), "page")
        ...
        reports = paginator_qry(rows, get_params.get("page"))
        late_in_early_out_ids = json.dumps(
            [row.id for row in reports.object_list if row.id]
        )
...
def search_attendance_requests(request):
    ...
    from attendance.views.requests import get_all_attendances_for_request_view
