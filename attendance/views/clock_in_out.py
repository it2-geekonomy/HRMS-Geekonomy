...
def late_come(attendance, start_time=None, end_time=None, shift=None):
    """
    Mark or clear late check-in based on punch time (from 9:11 AM), matching the
    Late / Early list UI. Shift start/grace args are kept for call-site compatibility.
    """
    if not enable_late_come_early_out_tracking(None).get("tracking"):
        return
    from attendance.late_punch import punch_flag_for_clock_in

    existing = AttendanceLateComeEarlyOut.objects.filter(
        type="late_come", attendance_id=attendance
    )
    if attendance.attendance_clock_in is None:
        for record in existing:
            if record.get_penalties_count() == 0:
                record.delete()
        return

    should_late = punch_flag_for_clock_in(attendance.attendance_clock_in) is not None
    if should_late:
...
def early_out(attendance, start_time=None, end_time=None, shift=None):
    """
    Mark or clear early check-out when punch-out is before 5:00 PM, matching the
    Late / Early list UI. Shift args are kept for call-site compatibility.
    """
    if not enable_late_come_early_out_tracking(None).get("tracking"):
        return

    from attendance.late_punch import is_early_out_clock
    ...
    should_early = is_early_out_clock(attendance.attendance_clock_out)
    if should_early:
