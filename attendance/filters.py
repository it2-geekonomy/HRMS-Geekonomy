...
class LateComeEarlyOutFilter(FilterSet):
    ...
    type = django_filters.ChoiceFilter(
        choices=AttendanceLateComeEarlyOut.choices,
        empty_label=_("All"),
    )
        ...
        for field in self.form.fields.keys():
            ...
            if field == "type":
                self.form.fields[field].widget.attrs["class"] = (
                    self.form.fields[field].widget.attrs.get("class", "")
                    + " oh-select oh-select--sm w-100"
                ).strip()
...
class AttendanceFilters(FilterSet):
    ...
    employee_active = django_filters.ChoiceFilter(
        ...
        method="filter_employee_active",
    )

    def filter_employee_active(self, queryset, name, value):
        """Default to active employees when filter is not sent (list + export)."""
        if self.data is not None and "employee_active" not in self.data:
        ...
        if value == "true":
            return queryset.filter(employee_id__is_active=True)
        if value == "false":
            return queryset.filter(employee_id__is_active=False)
        return queryset
...
def get_working_today(queryset, _name, value):
    ...
    if value:
        ...
        queryset = queryset.exclude(id__in=online_employee_ids)
    return queryset
...
def online_init(self, *args, **kwargs):
    ...
    custom_field = django_filters.BooleanFilter(
        label="Online", method=get_working_today
    )
