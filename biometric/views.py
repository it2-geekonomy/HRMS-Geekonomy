    ...
    def run(self):
        try:
            zk_device = ZK(
                ...
                force_udp=False,
                ommit_ping=True,  # Skip ping test since device blocks ping
            )
        ...
        if scheduler_form.is_valid():
            if device.machine_type == "zk":
                try:
                    ...
                    # Keep live mode enabled if it was already enabled
                    # device.is_live = False  # REMOVED: Allow both live and scheduler to be active
                    device.save()
...
def test_zkteco_connection(device):
    ...
    zk_device = ZK(
        machine_ip,
        port=port_no,
        timeout=120,  # Increased to 120 seconds for busy devices
        password=int(password),
        force_udp=False,
        ommit_ping=True,  # Skip ping test since device blocks ping
    )
...
def zk_employees_fetch(device):
    ...
    zk_device = ZK(
        device.machine_ip,
        port=device.port,
        timeout=30,  # Increased to 30 seconds for fetching users and templates
...
def find_employees_in_zk(device_id):
    ...
    zk_device = ZK(
        device.machine_ip, port=device.port, password=int(device.zk_password), timeout=30, ommit_ping=True
    )
    conn = zk_device.connect()
...
def delete_biometric_user(request, uid, device_id):
    """
    Remove the HRMS mapping for a biometric device user. Does not delete the user
    from the physical device so they can be re-mapped later.
    """
    device = BiometricDevices.objects.get(id=device_id)
    employee_bio = BiometricEmployees.objects.filter(
        uid=uid, device_id=device
    ).first()
    if not employee_bio:
        employee_bio = BiometricEmployees.objects.filter(
            user_id=str(uid), device_id=device
        ).first()
    if employee_bio:
        ...
        messages.success(
            request,
            _("Mapping removed for {}. The user is still enrolled on the device.").format(
                employee_name
            ),
        )
    else:
        messages.error(request, _("Biometric mapping not found."))
...
def bio_users_bulk_delete(request):
    """
    Remove HRMS mappings for multiple ZK device users. Does not delete users
...
def _fetch_zk_device_users(device):
    """Return all users enrolled on a ZK/eSSL device."""
    conn = None
    users = []
    try:
        zk_device = ZK(
        ...
        for user in conn.get_users():
            user_id = (
                str(user.user_id) if getattr(user, "user_id", None) is not None else None
            )
            if not user_id:
                continue
        ...
        logger.warning("Could not list ZK device users: %s", error)
    finally:
        if conn:
            try:
                conn.disconnect()
...
def get_zk_device_user_ids(device):
    """User IDs currently enrolled on the biometric device."""
    return {row["user_id"] for row in _fetch_zk_device_users(device)}


def get_employee_ids_linked_on_device(device, device_user_ids=None):
    """
    Employees whose HRMS mapping points to a user_id that still exists on the device.
    Orphan mappings (e.g. auto-assigned 1007 with no device user) are excluded so the
    employee can be chosen again in the map form.
    """
    if device_user_ids is None:
        device_user_ids = get_zk_device_user_ids(device)
    ...
    return set(
        BiometricEmployees.objects.filter(
            device_id=device, user_id__in=device_user_ids
        ).values_list("employee_id", flat=True)
    )


def get_unmapped_zk_device_users(device):
    """
    Device users on ZK/eSSL not yet linked in BiometricEmployees for this device.
    """
    mapped_user_ids = set(
        BiometricEmployees.objects.filter(device_id=device).values_list(
            "user_id", flat=True
    ...
    unmapped = []
    for row in _fetch_zk_device_users(device):
        if row["user_id"] not in mapped_user_ids:
            unmapped.append(row)
    return sorted(unmapped, key=lambda row: row["user_id"])
...
def map_biometric_users(request, device_id):
    ...
    if device.machine_type == "zk":
        unmapped_users = get_unmapped_zk_device_users(device)
        device_user_ids = get_zk_device_user_ids(device)
        form = MapZkBiometricUserForm(
            request.POST or None,
            device=device,
            unmapped_users=unmapped_users,
            linked_employee_ids=get_employee_ids_linked_on_device(
                device, device_user_ids=device_user_ids
            ),
            device_user_ids=device_user_ids,
        )
        if request.method == "POST" and form.is_valid():
            ...
            messages.success(
                request,
                _("Employee mapped to biometric device user successfully."),
            )
            return HttpResponse("<script>window.location.reload()</script>")
        return render(
            request,
            "biometric/map_zk_biometric_user.html",
            {
                "form": form,
                "device_id": device_id,
    ...
    if is_live:
        ...
        try:
            if device.machine_type == "zk":
                ...
                if conn:
                    device.is_live = True
                    # Keep scheduler enabled if it was already enabled
                    # device.is_scheduler = False
                    device.save()
                    instance.start()
            elif device.machine_type == "cosec":
...
def zk_biometric_attendance_logs(device_or_devices):
    ...
    for device in devices:
        ...
        zk_device = ZK(
            machine_ip,
            port=port_no,
            timeout=60,  # Increased to 60 seconds for fetching large datasets
            password=int(device.zk_password),
            force_udp=False,
            ommit_ping=True,  # Skip ping test since device blocks ping
        )
        ...
        try:
            ...
            # Update last fetch markers only if there are new records processed
            if filtered:
                # Update to 1 second after the latest processed scan to avoid reprocessing
                latest_processed_scan = max(filtered, key=lambda x: x.timestamp)
                updated_datetime = latest_processed_scan.timestamp + timedelta(seconds=1)
    ...
    for attendance in combined_attendances:
        ...
        # Use the device timestamp directly without timezone conversion
        # The device stores time in local time (IST), so we should use it as-is
        date_time = attendance.timestamp
        date = date_time.date()
        time = date_time.time()
        ...
        if bio_id:
            ...
            try:
                # Handle punch code 255 (unknown) by checking existing attendance
                if punch_code == 255:
                    ...
                    if existing_attendance and existing_attendance.attendance_clock_out is None:
                        # Employee has check-in but no check-out, so this is check-out
                        clock_out(request_data)
                    else:
                        # No attendance or already has check-out, so this is check-in
                        clock_in(request_data)
                elif punch_code in {0, 3, 4}:
                    clock_in(request_data)
...
try:
    # Only disable live mode for devices that are starting the scheduler
    # devices = BiometricDevices.objects.all().update(is_live=False)  # REMOVED: This was disabling all devices
