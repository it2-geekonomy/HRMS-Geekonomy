...
class MapZkBiometricUserForm(Form):
    """
    Map an existing ZK/eSSL device user to a Horilla employee (no new device enrollment).
    """

    device_user = forms.ChoiceField(
        label=_("Device user"),
        required=False,
        help_text=_(
            "Select a user from the device who is not yet linked in HRMS."
        ),
    )
    user_id_manual = forms.CharField(
        label=_("Device User ID"),
        required=False,
        max_length=100,
        widget=forms.TextInput(attrs={"class": "oh-input w-100"}),
        help_text=_(
            "Use when the list above is empty or the device could not be reached."
        ),
    )
    employee_id = forms.ModelChoiceField(
        queryset=Employee.objects.none(),
        label=_("Horilla employee"),
        widget=forms.Select(
    ...
    def __init__(
        ...
        *args,
        device=None,
        unmapped_users=None,
        linked_employee_ids=None,
        device_user_ids=None,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.device = device
        self.device_user_ids = device_user_ids or set()
        choices = [("", _("--- Choose device user ---"))]
        for entry in unmapped_users or []:
            uid = entry.get("uid")
            user_id = str(entry["user_id"])
            value = f"{user_id}|{uid if uid is not None else ''}"
            label = entry.get("name") or user_id
            choices.append((value, f"{label} (User ID: {user_id})"))
        self.fields["device_user"].choices = choices

        if device:
            if linked_employee_ids is None:
                linked_employee_ids = BiometricEmployees.objects.filter(
                    device_id=device
                ).values_list("employee_id", flat=True)
            self.fields["employee_id"].queryset = Employee.objects.filter(
                is_active=True
            ).exclude(id__in=linked_employee_ids)

    def clean(self):
        cleaned_data = super().clean()
        device_user = (cleaned_data.get("device_user") or "").strip()
        user_id_manual = (cleaned_data.get("user_id_manual") or "").strip()
        employee = cleaned_data.get("employee_id")

        user_id = None
        uid = None
        if device_user:
            parts = device_user.split("|", 1)
            ...
            raise forms.ValidationError(
                _("Select a device user or enter a Device User ID.")
            )

        if not self.device:
            raise forms.ValidationError(_("Biometric device not found."))

        if BiometricEmployees.objects.filter(
            device_id=self.device, user_id=str(user_id)
        ).exists():
            raise forms.ValidationError(
                {
                    "device_user": _(
                        "User ID %(user_id)s is already mapped on this device."
                    )
                    % {"user_id": user_id}
                }
            )
        ...
        if employee:
            existing = BiometricEmployees.objects.filter(
                device_id=self.device, employee_id=employee
            ).first()
            if existing and str(existing.user_id) in self.device_user_ids:
                if str(existing.user_id) != str(user_id):
                    raise forms.ValidationError(
                        {
                            "employee_id": _(
                                "This employee is already mapped on this device."
                            )
                        }
                    )

        cleaned_data["user_id"] = str(user_id)
