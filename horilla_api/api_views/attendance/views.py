...
from attendance.models import Attendance, AttendanceActivity, AttendanceRequestLog, EmployeeShiftDay
from attendance.threading import send_attendance_request_approved_emails
    ...
    def put(self, request, pk):
        try:
            ...
            if attendance.requested_data is not None:
                ...
                Attendance.objects.entire().filter(id=pk).update(**requested_data)
            ...
            _log_attendance_request_action(
                ...
                description=_("Approved for %(date)s")
            ...
            try:
                ...
                logger.exception(
                    "Attendance request approved email failed (API): %s", e
                )
        except Exception as E:
            return Response({"error": str(E)}, status=400)
