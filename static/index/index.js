...
// Add CSS globally once for list display
$(document).ready(function() {
    if (!$('#summernote-list-fix').length) {
        $('<style id="summernote-list-fix">.note-editable ul, .note-editable ul ul { list-style-type: disc !important; padding-left: 40px !important; } .note-editable ol, .note-editable ol ol { list-style-type: decimal !important; padding-left: 40px !important; } .note-editable li { display: list-item !important; list-style-position: outside !important; }</style>').appendTo('head');
    }
});
    ...
    if ($("[data-summernote]").length > 0) {
        ...
        setTimeout(function() {
            $('.note-editor').each(function() {
                var $editor = $(this);
                if (!$editor.siblings('.preview-btn').length) {
                    $editor.after('<button type="button" class="preview-btn oh-btn oh-btn--secondary oh-btn--small mt-2">Preview</button>');
                }
            });
        }, 300);
    }
...
$(document).on('click', '.preview-btn', function() {
    var content = $(this).siblings('.note-editor').find('.note-editable').html();
    Swal.fire({
        title: 'Preview',
        html: '<div style="text-align: left; max-height: 400px; overflow-y: auto; padding: 10px;"><style>ul, ul ul { list-style-type: disc !important; padding-left: 40px !important; } ol, ol ol { list-style-type: decimal !important; padding-left: 40px !important; } li { display: list-item !important; list-style-position: outside !important; }</style>' + (content || 'No content') + '</div>',
        width: '600px',
