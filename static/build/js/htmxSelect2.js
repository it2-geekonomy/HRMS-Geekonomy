...
// Helper function to hash data using SHA-256 (falls back on HTTP / non-secure contexts)
async function hashData(data) {
    if (!window.crypto || !window.crypto.subtle) {
        return "plain_" + String((data || "").length);
    }
...
async function saveToLocalStorage(data) {
    ...
    if (!window.crypto || !window.crypto.subtle) {
        localStorage.setItem("cachedInstalledAppsHash", "plain_" + String(dataString.length));
        return;
    }
...
async function loadFromLocalStorage() {
    ...
    if (!window.crypto || !window.crypto.subtle) {
        if (storedHash === "plain_" + String(dataString.length)) {
            return JSON.parse(dataString);
        }
        return null;
    }
...
$(document).on("htmx:afterSwap", async function (evt) {
    ...
    try {
            ...
            $.ajax({
                ...
                success: async function (response) {
                    ...
                    try {
                        ...
                        console.warn("Could not cache installed apps:", storageError);
                    }
                    loadScripts(cachedInstalledApps);
                },
        ...
        console.warn("Installed apps cache skipped:", cacheError);
    }
});
