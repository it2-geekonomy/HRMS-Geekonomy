...
class Contract(HorillaModel):
    ...
    def get_display_name(self):
        """Contract title using current employee name and badge (e.g. GEEKY0029), not stored contract_name."""
        return f"{self.employee_id}'s Contract"
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            ...
            and self.contract_end_date < date.today()
            and self.contract_status == "active"
        ):
...
class Allowance(HorillaModel):
    ...
    based_on_choice = [
        ...
        (
            "special_allowance",
            _("Special Allowance (Total Salary - Basic - HRA - Conveyance - Medical)"),
        ),
    ]
...
class MonthlySalaryData(HorillaModel):
    """
    Stores salary calculation data per employee per month.
    Formula: calculated_salary = (days_worked / working_days) * monthly_salary
    where days_worked = P (Present) + L (Paid Leave) from Attendance Calendar,
    ...
    employee_id = models.ForeignKey(
        ...
        on_delete=models.PROTECT,
        verbose_name=_("Employee"),
        related_name="monthly_salary_data_set",
    )
    ...
    days_worked = models.FloatField(
        ...
        help_text=_("Count of P (Present) + L (Paid Leave) from attendance calendar"),
    )
    working_days = models.IntegerField(
        default=0,
        verbose_name=_("Working Days"),
        help_text=_("Total days in month excluding PH and WO"),
    )
    ...
    monthly_salary = models.FloatField(
        ...
        help_text=_("Basic salary × 2"),
    )
    calculated_salary = models.FloatField(
        default=0,
        verbose_name=_("Calculated Salary"),
        help_text=_("(days_worked / working_days) × monthly_salary, before PT"),
    )
    pt_deduction = models.FloatField(
        default=200,
        verbose_name=_("PT (Professional Tax)"),
        help_text=_("Fixed deduction, default ₹200"),
    )
    final_salary = models.FloatField(
        default=0,
        verbose_name=_("Final Salary"),
        help_text=_("Calculated salary minus PT"),
    )
    arrears_amount = models.FloatField(
        default=0,
        blank=True,
        verbose_name=_("Arrears Amount"),
        help_text=_("Amount added to final salary (e.g. one-time arrears)"),
    )
    arrears_description = models.CharField(
        max_length=255,
        blank=True,
        default="",
        verbose_name=_("Arrears Description"),
    )
    archived = models.BooleanField(
        default=False,
        verbose_name=_("Archived"),
        help_text=_("Archived data is hidden from the default list and only shown when filtering by month/year."),
    )

    objects = HorillaCompanyManager("employee_id__employee_work_info__company_id")

    def total_pay(self):
        """Final salary plus arrears (for display)."""
        return (self.final_salary or 0) + (self.arrears_amount or 0)

    class Meta:
        ordering = ["-year", "-month", "employee_id"]
        unique_together = [("employee_id", "year", "month")]
        verbose_name = _("Monthly Salary Data")
        verbose_name_plural = _("Monthly Salary Data")

    def __str__(self):
        return f"{self.employee_id} - {self.year}-{self.month:02d}"


class SalaryDataArrearsLog(models.Model):
    """
    Log of who added/updated arrears for a MonthlySalaryData record.
    """
    ...
    action = models.CharField(
        max_length=20,
        choices=[("added", _("Added")), ("updated", _("Updated"))],
        default="updated",
    )
    ...
    class Meta:
        ordering = ["-created_at"]
        verbose_name = _("Arrears Log")
        verbose_name_plural = _("Arrears Logs")

    def __str__(self):
        return f"{self.salary_data} - {self.action} ₹{self.amount} by {self.user}"


class SalaryDataAuditLog(models.Model):
    """
    Who generated or deleted Salary Data, when (for compliance / support).
    """

    ACTION_GENERATED = "generated"
    ACTION_DELETED_SINGLE = "deleted_single"
    ACTION_DELETED_BULK = "deleted_bulk"
    ACTION_CHOICES = [
        (ACTION_GENERATED, _("Generated")),
        (ACTION_DELETED_SINGLE, _("Deleted (single)")),
        (ACTION_DELETED_BULK, _("Deleted (bulk)")),
    ]
    ...
    user = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="salary_data_audit_logs",
        verbose_name=_("User"),
    )
    action = models.CharField(
        max_length=20,
    ...
    month = models.IntegerField(null=True, blank=True, verbose_name=_("Month"))
    summary = models.TextField(blank=True, default="", verbose_name=_("Details"))
    ...
    class Meta:
        ...
        verbose_name = _("Salary Data activity log")
        verbose_name_plural = _("Salary Data activity logs")

    def __str__(self):
        return f"{self.get_action_display()} @ {self.performed_at} by {self.user}"
...
class Expense(HorillaModel):
    """
    Payroll expense tracking — records who added, date, purpose, payment details,
    optional receipt, and archive status.
    """
    ...
    expense_date = models.DateField(verbose_name=_("Date"))
    description = models.TextField(
        max_length=500,
        blank=True,
        default="",
        verbose_name=_("Description / Purpose"),
    )
    ...
    attachment = models.FileField(
        ...
        null=True,
        blank=True,
        verbose_name=_("Receipt / Document"),
    )
    archived = models.BooleanField(
        default=False,
        verbose_name=_("Archived"),
        help_text=_("Archived expenses are hidden from the default list."),
    )

    objects = HorillaCompanyManager("added_by__employee_work_info__company_id")
    ...
    def total_amount(self):
        return sum((line.amount or 0) for line in self.breakups.all())

    def grouped_breakups(self):
        from collections import OrderedDict

        groups = OrderedDict()
        for line in self.breakups.all():
            category = line.category or _("General")
            groups.setdefault(category, []).append(line)
        return list(groups.items())

    def __str__(self):
        summary = (self.description or "")[:40] or _("No description")
        return f"{self.added_by} — {self.expense_date} — {summary}"


class ExpenseBreakup(models.Model):
    """Line-item amount breakup for an expense, grouped by category."""
    ...
    class Meta:
        ordering = ["id"]
        verbose_name = _("Expense Breakup")
        verbose_name_plural = _("Expense Breakups")
