...
def daily_computation(employee, wage, start_date, end_date):
    ...
    Company uses 30 days formula: All holidays/Sundays are paid.
    LOP only reduces for absent days, not holidays.
    ...
    Args:
        ...
        end_date (obj): end date of the period
    """
    # Use fixed 30 days per month (company policy: holidays/Sundays are paid)
    # Calculate number of months in the period
    from dateutil.relativedelta import relativedelta
    from calendar import monthrange
    
    # Calculate months covered (for multi-month periods)
    ...
    while current_date <= end_date:
        ...
        # Use 30 days per month (company formula)
        months_count += 1
        current_date = (current_date + relativedelta(months=1)).replace(day=1)
    
    # Total working days = 30 days per month (holidays/Sundays are paid)
    # Always use 30 days per month, regardless of actual calendar days
    total_working_days = 30 * months_count
    ...
    # Paid days = Total working days (30) - LOP days only
    # Holidays/Sundays are included in paid days
...
def months_between_range(wage, start_date, end_date):
    ...
    for current_date in (
        ...
        # For partial month, calculate proportionally but cap at 30
        days_in_period = (current_end_date - month_start_date).days + 1
        total_working_days_on_period = min(days_in_period, 30)
...
def monthly_computation(employee, wage, start_date, end_date, *args, **kwargs):
    ...
    Basic pay = contract basic salary (wage) prorated for the period.

    Args:
        employee (obj): Employee instance
        wage (float): wage of the employee (basic salary from contract)
        start_date (obj): start of the pay period
        end_date (obj): end date of the period
    """
    basic_pay = 0
    ...
    # Use 30 days per month (company policy: holidays/Sundays are paid)
    # Paid days = 30 - LOP days only
    total_working_days_for_period = 30  # Fixed 30 days per month
    paid_days = total_working_days_for_period - unpaid_leaves
