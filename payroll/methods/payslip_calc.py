...
def calculate_allowance(**kwargs):
    ...
    for allowance in allowances:
        if allowance.is_condition_based:
            ...
            if allowance.based_on in filter_mapping:
                ...
                employee_allowances.append(allowance)
    # Separate Special Allowance (computed last from total - basic - HRA - Conveyance - Medical)
    special_allowances = [a for a in employee_allowances if getattr(a, "based_on", None) == "special_allowance"]
    regular_allowances = [a for a in employee_allowances if getattr(a, "based_on", None) != "special_allowance"]

    # Filter and append taxable allowance and not taxable allowance (excluding special)
    for allowance in regular_allowances:
    ...
    # Special Allowance = monthly salary - (basic + hra + medical + lta)
    total_allowance_so_far = sum(a["amount"] for a in serialized_allowances)
    special_allowance_amount = None
    for allowance in special_allowances:
        amount = calculate_based_on_special_allowance(
            **{
                "employee": employee,
                "start_date": start_date,
                "end_date": end_date,
                ...
                "total_allowance": total_allowance_so_far,
                "contract_wage": kwargs.get("contract_wage"),
        ...
        kwargs["component"] = allowance
        amount = if_condition_on(**kwargs)
        special_allowance_amount = amount
        serialized_allowances.append({
            "allowance_id": allowance.id,
            "title": allowance.title,
            "is_taxable": allowance.is_taxable,
            "amount": amount,
    ...
    # If no Special Allowance component exists, always add it so it appears on payslip
    if not special_allowances and (kwargs.get("contract_wage") is not None or kwargs.get("contract") is not None):
        special_allowance_amount = calculate_based_on_special_allowance(
            **{
                "employee": employee,
                "basic_pay": basic_pay,
                "allowances": serialized_allowances,
        ...
        serialized_allowances.append({
            ...
            "is_taxable": True,
            "amount": special_allowance_amount,
        })
...
# Title patterns for Special Allowance formula: special = monthly_salary - (basic + hra + medical + lta)
# Fallback: when basic = 0.50 * monthly CTC, infer monthly_salary = basic_pay / BASIC_PCT_OF_CTC
BASIC_PCT_OF_CTC = 0.50

SPECIAL_ALLOWANCE_HRA_TITLES = ("house rent allowance", "hra")
SPECIAL_ALLOWANCE_CONVEYANCE_TITLES = (
    "conveyance allowance / lta",
    "conveyance allowance",
    "conveyance",
    "lta",
)
SPECIAL_ALLOWANCE_MEDICAL_TITLES = ("medical allowance", "medical")


def calculate_based_on_special_allowance(*_args, **kwargs):
    """
    Special Allowance = monthly_salary - (basic + HRA + medical + LTA).
    Uses contract wage as monthly salary so Special is the remainder that makes
    basic + HRA + medical + LTA + Special = monthly salary.
    """
    employee = kwargs.get("employee")
    basic_pay = kwargs.get("basic_pay") or 0
    allowances_list = kwargs.get("allowances") or []
    contract_wage = kwargs.get("contract_wage")
    contract = kwargs.get("contract")
    ...
    for item in allowances_list:
        ...
        if _match_title(title, SPECIAL_ALLOWANCE_HRA_TITLES):
            hra_amt += amt
        elif _match_title(title, SPECIAL_ALLOWANCE_CONVEYANCE_TITLES):
            conveyance_amt += amt
        elif _match_title(title, SPECIAL_ALLOWANCE_MEDICAL_TITLES):
            medical_amt += amt

    # Monthly CTC = contract basic (wage) * 2 — same as Salary Data module.
    monthly_salary = None
    if contract is not None and contract.wage is not None:
        wage = float(contract.wage)
        if getattr(contract, "wage_type", None) == "daily":
            monthly_salary = wage * 30
        else:
            monthly_salary = wage * 2
    elif contract_wage is not None:
        monthly_salary = float(contract_wage)
    elif employee:
        contract = Contract.objects.filter(
            employee_id=employee, contract_status="active"
        ).first()
        if contract and contract.wage is not None:
            wage = float(contract.wage)
            if getattr(contract, "wage_type", None) == "monthly":
                monthly_salary = wage
            elif getattr(contract, "wage_type", None) == "daily":
                monthly_salary = wage * 30
            else:
                monthly_salary = wage

    sum_components = basic_pay + hra_amt + conveyance_amt + medical_amt

    # If contract wage is missing or stored as "sum of components" (so Special would be 0),
    # infer monthly CTC from basic: basic = 0.50 * monthly CTC => monthly_salary = basic_pay / 0.50
    if monthly_salary is None or monthly_salary <= sum_components:
        if basic_pay and BASIC_PCT_OF_CTC:
            monthly_salary = basic_pay / BASIC_PCT_OF_CTC
        elif monthly_salary is None:
            return 0.0

    # Special Allowance = monthly salary - (basic + hra + medical + lta)
    special = monthly_salary - sum_components
    return max(0.0, round(special, 2))
...
calculation_mapping = {
    ...
    "special_allowance": calculate_based_on_special_allowance,
}
