...
class GenerateSalaryDataForm(HorillaForm):
    """
    Form to generate monthly salary data (single or bulk).
    Uses Attendance Calendar: days_worked = P + L, working_days = exclude PH & WO,
    monthly_salary = basic_salary * 2, calculated_salary = (days_worked / working_days) * monthly_salary.
    Employee filter (when you click the field): First Name, Last Name, Is Active? only — no Work Info.
    """
    ...
    year = forms.IntegerField(
        ...
        min_value=2020,
        max_value=2100,
        widget=forms.NumberInput(attrs={"class": "oh-input w-100", "placeholder": "e.g. 2026"}),
    )
    month = forms.TypedChoiceField(
        label=_("Month"),
        coerce=int,
        choices=[(i, calendar.month_abbr[i]) for i in range(1, 13)],
        widget=forms.Select(attrs={"class": "oh-input w-100 oh-select"}),
    )
    employee_id = HorillaMultiSelectField(
        queryset=Employee.objects.none(),
        widget=HorillaMultiSelectWidget(
    ...
    def clean(self):
        ...
        if year and month:
            ...
            if year > today.year or (year == today.year and month > today.month):
                raise forms.ValidationError(
                    _("Month and year cannot be in the future.")
                )
        return cleaned_data

    class Meta:
        fields = ("year", "month", "employee_id")


class MonthlySalaryDataEditForm(ModelForm):
    """
    Edit form for MonthlySalaryData.
    We allow editing the inputs (days_worked, working_days, basic_salary, pt),
    and recompute monthly_salary/calculated_salary/final_salary on save.
    """

    class Meta:
        model = MonthlySalaryData
        fields = ("days_worked", "working_days", "basic_salary", "pt_deduction")
...
class ExpenseBreakupForm(ModelForm):
    class Meta:
        model = ExpenseBreakup
        fields = ["particular", "amount"]
        widgets = {
            "particular": forms.TextInput(
                attrs={
                    "class": "oh-input w-100",
                    "placeholder": _("e.g. Travel, Food, Supplies"),
                }
            ),
...
def get_expense_breakup_formset(instance=None, data=None):
    """New expenses get one empty line; edit shows only saved lines."""
    extra = 0 if instance and instance.pk else 1
    formset_class = inlineformset_factory(
        Expense,
        ExpenseBreakup,
        form=ExpenseBreakupForm,
        extra=extra,
        can_delete=True,
        min_num=1,
        validate_min=True,
    ...
    return formset_class(instance=instance)


class ExpenseForm(ModelForm):
    """Form for creating and editing payroll expenses."""
    ...
    class Meta:
        ...
        widgets = {
            ...
            "mode_of_payment": forms.TextInput(
                attrs={
                    "class": "oh-input w-100",
                    "placeholder": _("e.g. UPI, Bank transfer, Cash"),
                }
            ),
            "paid_by": forms.TextInput(
                attrs={
                    "class": "oh-input w-100",
                    "placeholder": _("e.g. Company card, Sachin Nayak"),
                }
            ),
        }
        error_messages = {
            "added_by": {
                "required": _("Please select who added this expense."),
            },
            "expense_date": {
                "required": _("Please select the expense date."),
            },
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not kwargs.get("data"):
            ...
            if instance is not None and instance.expense_date is not None:
                initial["expense_date"] = instance.expense_date.strftime("%Y-%m-%d")
            elif instance is None or not instance.pk:
                initial.setdefault(
        ...
        reload_queryset(self.fields)
        if not self.instance.pk:
            if self.request and getattr(self.request.user, "employee_get", None):
                self.initial["added_by"] = self.request.user.employee_get.id
        if self.request and not self.request.user.has_perm("payroll.add_expense"):
            self.fields.pop("added_by", None)
        self.fields["attachment"].widget.attrs["accept"] = ".jpg,.jpeg,.png,.pdf,.doc,.docx"

    def save(self, commit=True):
        instance = super().save(commit=False)
        if not instance.added_by_id and self.request:
            instance.added_by = getattr(self.request.user, "employee_get", None)
        if commit:
            instance.save()
        return instance
