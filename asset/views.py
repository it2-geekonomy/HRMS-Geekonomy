...
def asset_del(request, asset):
    ...
    Removes any returned allocations (return_date set) so the asset can be deleted
    when there is no active allocation.
    """
    try:
        # Delete returned allocation records so PROTECT does not block asset delete
        AssetAssignment.objects.filter(asset_id=asset).exclude(
...
def asset_delete(request, asset_id):
    ...
    # Block delete only when asset has an active (not yet returned) allocation
    has_active_allocation = AssetAssignment.objects.filter(
...
def asset_list(request, cat_id):
    ...
    asset_list = asset_filtered.qs.filter(asset_category_id=cat_id).select_related(
        'asset_category_id', 
        'owner'
    ).prefetch_related(
...
def asset_request_creation(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Notify the requesting employee's reporting manager
            try:
                requested_employee = asset_request.requested_employee_id
                reporting_manager = getattr(
                    getattr(requested_employee, "employee_work_info", None),
                    "reporting_manager_id",
                    None,
                )
                if reporting_manager and getattr(
                    reporting_manager, "employee_user_id", None
                ):
                    sender = getattr(request.user, "employee_get", None) or requested_employee
                    notify.send(
                        sender,
                        recipient=reporting_manager.employee_user_id,
                        verb=_("An asset request has been submitted for your approval."),
...
def asset_allocate_return_request(request, asset_id):
    ...
    notify.send(
        ...
        verb=f"Return request for {asset_assign.asset_id} initiated from\
            {asset_assign.get_assignee_name()}",
        verb_ar=f"تم بدء طلب الإرجاع للمورد {asset_assign.asset_id}\
            من {asset_assign.get_assignee_name()}",
        verb_de=f"Rückgabewunsch für {asset_assign.asset_id} von\
            {asset_assign.get_assignee_name()} initiiert",
        verb_es=f"Solicitud de devolución para {asset_assign.asset_id}\
            iniciada por {asset_assign.get_assignee_name()}",
        verb_fr=f"Demande de retour pour {asset_assign.asset_id}\
            initiée par {asset_assign.get_assignee_name()}",
        redirect=reverse("asset-request-allocation-view")
        + f"?assigned_to_employee_id={asset_assign.assigned_to_employee_id_id}&\
...
@login_required
@permission_required(perm="asset.delete_assetassignment")
def asset_allocation_delete(request, allocation_id):
    """
    Delete a returned asset allocation. Only allocations with return_date set can be deleted.
    Data is retained in Asset History; this removes the record from the Allocation list.
    """
    allocation = get_object_or_404(AssetAssignment, id=allocation_id)
    if not allocation.return_date:
        messages.error(
            request,
            _("Only returned allocations can be deleted. Return the asset first."),
        )
    ...
    allocation.delete()
    messages.success(request, _("Asset allocation deleted."))
    if request.headers.get("HX-Request"):
        context = filter_pagination_asset_request_allocation(request)
        template = "request_allocation/asset_request_allocation_list.html"
        if (
...
def _parse_import_date(val):
    """Return a date for asset import; use today if missing or invalid."""
...
def _parse_import_decimal(val, default=0):
    """Return a decimal for asset import (e.g. cost); use default if missing or invalid."""
    if val is None or (isinstance(val, float) and pd.isna(val)):
        return default
    if isinstance(val, str) and not str(val).strip():
        return default
...
def csv_asset_import(file):
    ...
    with open(tmp_file, errors="ignore") as csv_file:
        ...
        for row_index, row in enumerate(reader):
            ...
            asset_name = (asset_name or "").strip() or f"Asset-{row_index + 1}"
            raw_id = (asset_tracking_id or "").strip() or f"IMPORT-{row_index + 1}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
            asset_tracking_id = raw_id[:30]
            purchase_date = _parse_import_date(asset_purchase_date if asset_purchase_date else None)
            purchase_cost = _parse_import_decimal(asset_purchase_cost if asset_purchase_cost else None)
            asset_status = (asset_status or "").strip() or "Available"

            # Helper function to get or create categories and lots
            asset_category, _ = AssetCategory.objects.get_or_create(
                asset_category_name=asset_category_name or "",
                defaults={"asset_category_description": ""},
            )
            asset_lot = None
            if asset_lot_number and str(asset_lot_number).strip():
                lot_str = str(asset_lot_number).strip()[:30]
                asset_lot, _ = AssetLot.objects.get_or_create(
                    lot_number=lot_str,
                    defaults={"lot_description": ""},
                )
            asset_list.append(
                Asset(
...
def spreadsheetml_asset_import(dataframe):
    for index, row in dataframe.iterrows():
        asset_name = convert_nan(row["Asset name"]) or ""
        asset_description = convert_nan(row["Description"])
        raw_tracking_id = convert_nan(row["Tracking id"])
        purchase_date = _parse_import_date(convert_nan(row["Purchase date"]))
        purchase_cost = _parse_import_decimal(convert_nan(row["Purchase cost"]))
        category_name = convert_nan(row["Category"]) or ""
        lot_number = convert_nan(row["Batch number"])
        status = convert_nan(row["Status"]) or "Available"

        if not asset_name or not str(asset_name).strip():
            asset_name = f"Asset-{index + 1}"
        raw_id = (
            raw_tracking_id
            if raw_tracking_id and str(raw_tracking_id).strip()
            else f"IMPORT-{index + 1}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
        )
        asset_tracking_id = (str(raw_id).strip())[:30]

        asset_category, create = AssetCategory.objects.get_or_create(
            asset_category_name=(category_name[:255] if len(category_name) > 255 else category_name),
            defaults={"asset_category_description": ""},
        )
        asset_lot_number = None
        if lot_number is not None and str(lot_number).strip():
            lot_str = str(lot_number).strip()[:30]
            asset_lot_number, _ = AssetLot.objects.get_or_create(
                lot_number=lot_str,
                defaults={"lot_description": ""},
            )
...
def _assets_in_use_queryset():
    """
    Assets that are "in use": either have asset_status="In use" (as in Asset
    Category list) or have at least one active assignment (not returned).
    Match status case-insensitively in case DB has "In Use" etc.
    """
    assets_in_use = Asset.objects.filter(
        Q(asset_status__iexact="in use")
        | Q(
            assetassignment__return_date__isnull=True,
            assetassignment__assigned_to_employee_id__is_active=True,
        )
...
def asset_dashboard(request):
    ...
    """
    assets = Asset.objects.all()
    asset_in_use = _assets_in_use_queryset()
    ...
    context = {
        ...
        "asset_in_use_count": asset_in_use.count(),
...
def asset_dashboard_allocates(request):
    # Show all active allocations (not yet returned), not only those with asset_status="In use"
    asset_allocations = AssetAssignment.objects.filter(
        return_date__isnull=True, assigned_to_employee_id__is_active=True
    )
    context = {
...
@hx_request_required
@permission_required(perm="asset.view_assetcategory")
def asset_dashboard_card_modal(request, card_type):
    """
    Return HTML fragment for asset dashboard card modal.
    card_type: 'assets' | 'requests' | 'in_use'
    """
    if card_type == "assets":
        assets = Asset.objects.all().select_related("asset_category_id")[:100]
        context = {"card_type": "assets", "assets": assets}
    elif card_type == "requests":
        asset_requests = AssetRequest.objects.filter(
            asset_request_status="Requested", requested_employee_id__is_active=True
        ).select_related("requested_employee_id", "asset_category_id")[:100]
        context = {"card_type": "requests", "asset_requests": asset_requests}
    elif card_type == "in_use":
        asset_in_use = _assets_in_use_queryset()[:100]
        context = {"card_type": "in_use", "asset_in_use": asset_in_use}
    else:
        context = {"card_type": card_type}
    html = render_to_string(
        "asset/dashboard/dashboard_card_modal_content.html",
        context,
        request=request,
    )
...
def asset_available_chart(_request):
    ...
    dataset = [
        {
            "label": _("asset"),
            "data": [asset_in_use.count(), len(asset_available), len(asset_unavailable)],
        },
    ]
...
def asset_category_chart(_request):
    ...
    for asset_category in asset_categories:
        # Count assets in this category that are in use (by status or active assignment)
        category_count = asset_category.asset_set.filter(
            Q(asset_status__iexact="in use")
            | Q(
                assetassignment__return_date__isnull=True,
                assetassignment__assigned_to_employee_id__is_active=True,
