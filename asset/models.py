...
class Asset(HorillaModel):
    ...
    asset_tracking_id = models.CharField(
        max_length=30, null=False, unique=True, verbose_name=_("Asset Id")
    )
    ...
    def clean(self):
        ...
        if existing_asset.exists():
            raise ValidationError(
                {
                    "asset_tracking_id": _(
                        "An asset with this Asset Id already exists."
                    )
                }
            )
        return super().clean()
...
class AssetAssignment(HorillaModel):
    ...
    is_temporary_allocation = models.BooleanField(
        default=False,
        verbose_name=_("Temporary Allocation"),
    )
    temporary_assignee_name = models.CharField(
        max_length=150,
        blank=True,
        default="",
        verbose_name=_("Temporary Assignee Name"),
    )
    ...
    def get_assignee_avatar(self):
        from django.templatetags.static import static

        if self.assigned_to_employee_id:
            return self.assigned_to_employee_id.get_avatar()
        return static("images/ui/default_avatar.jpg")

    def get_assignee_name(self):
        if self.temporary_assignee_name and (
            self.is_temporary_allocation or not self.assigned_to_employee_id
        ):
            return f"{self.temporary_assignee_name} ({_('Temporary')})"
        if self.assigned_to_employee_id:
            return self.assigned_to_employee_id.get_full_name()
        return _("—")

    def save(self, *args, **kwargs):
        if self.temporary_assignee_name and not self.assigned_to_employee_id:
            self.is_temporary_allocation = True
        elif self.assigned_to_employee_id:
            self.is_temporary_allocation = False
            if not self.temporary_assignee_name:
                self.temporary_assignee_name = ""
        super().save(*args, **kwargs)

    def __str__(self):
        return f"{self.get_assignee_name()} --- {self.asset_id} --- {self.return_status}"
