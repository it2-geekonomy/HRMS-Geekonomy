...
class AssetForm(ModelForm):
    ...
    Batch No and Expiry Date are excluded; Description is optional.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        uuid_map = {
            ...
            for field in ["asset_category_id", "asset_status"]
        }
    ...
    def clean(self):
        ...
        if instance.pk:
            ...
            if (
                ...
                raise ValidationError(
                    {"asset_tracking_id": _("An asset with this Asset Id already exists.")}
                )
...
class AssetRequestForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Show only Asset Categories that have at least one asset available (not in use)
        self.fields["asset_category_id"].queryset = AssetCategory.objects.filter(
            asset__asset_status="Available"
        ).distinct()
...
class AssetAllocationForm(ModelForm):
    ...
    ALLOCATION_TYPE_CHOICES = [
        ("employee", _("Employee")),
        ("temporary", _("Temporary allocation (non-employee)")),
    ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["assigned_to_employee_id"].required = False
        self.fields["temporary_assignee_name"].required = False
        if self.instance.pk and self.instance.is_temporary_allocation:
            self.fields["allocation_type"].initial = "temporary"

        self.fields["assign_images"] = MultipleFileField(
            label=_("Assign Condition Images"),
            required=False,
        )
        self.fields["temporary_assignee_name"].widget.attrs.update(
            {
                "class": "oh-input w-100",
                "placeholder": _("Enter name of temporary assignee"),
            }
        )

    def clean(self):
        cleaned_data = super().clean()
        allocation_type = cleaned_data.get("allocation_type", "employee")
        ...
        if allocation_type == "temporary":
            if not temp_name:
                raise ValidationError(
                    {
                        "temporary_assignee_name": _(
                            "Name is required for temporary allocation."
                        )
                    }
                )
            cleaned_data["assigned_to_employee_id"] = None
            cleaned_data["is_temporary_allocation"] = True
            cleaned_data["temporary_assignee_name"] = temp_name
        else:
            if not employee:
                raise ValidationError(
                    {
                        "assigned_to_employee_id": _(
                            "Please select an employee."
                        )
                    }
                )
            cleaned_data["is_temporary_allocation"] = False
            cleaned_data["temporary_assignee_name"] = ""
        return cleaned_data

    def save(self, commit=True):
        instance = super().save(commit=False)
        allocation_type = self.cleaned_data.get("allocation_type", "employee")
        if allocation_type == "temporary":
            instance.is_temporary_allocation = True
            instance.assigned_to_employee_id = None
            instance.temporary_assignee_name = (
                self.cleaned_data.get("temporary_assignee_name") or ""
            ).strip()
        else:
            instance.is_temporary_allocation = False
            instance.temporary_assignee_name = ""
        if commit:
            instance.save()
            self.save_m2m()
