...
def onboarding_view(request):
    """
    Simplified onboarding: list of converted employees with checklist.
    Supports filter by archived status and joining date.
    """
    progress_list = OnboardingProgress.objects.select_related(
        "employee_id",
        "employee_id__employee_work_info",
        "employee_id__employee_work_info__job_position_id",
    ).order_by("-employee_id__id")

    # Filter: archived (active / archived / all)
    archived_param = request.GET.get("archived", "").strip().lower()
    if archived_param == "true" or archived_param == "1":
        progress_list = progress_list.filter(is_archived=True)
    elif archived_param == "all":
        pass
    else:
        progress_list = progress_list.filter(is_archived=False)

    # Filter: joining date range (employee_work_info.date_joining)
    joining_from = request.GET.get("joining_date_from", "").strip()
    joining_to = request.GET.get("joining_date_to", "").strip()
    if joining_from:
        try:
            from datetime import datetime
            dt = datetime.strptime(joining_from, "%Y-%m-%d").date()
            progress_list = progress_list.filter(
                employee_id__employee_work_info__date_joining__gte=dt
            )
    ...
    if joining_to:
        try:
            from datetime import datetime
            dt = datetime.strptime(joining_to, "%Y-%m-%d").date()
            progress_list = progress_list.filter(
                employee_id__employee_work_info__date_joining__lte=dt
            )
        except (ValueError, TypeError):
            pass

    progress_list = paginator_qry(progress_list, request.GET.get("page"))
    step_labels = [
        (_("Offer Letter Issued & Accepted"), "step_1"),
        (_("Joining Date Confirmed"), "step_2"),
        (_("Personal Details Form Collected"), "step_3"),
        (_("ID Proof & Address Proof Received"), "step_4"),
        (_("Education Certificates Received"), "step_5"),
        (_("Previous Employment Proof / Pay Slips Collected"), "step_6"),
        (_("PF / ESIC / UAN Details Collected (if applicable)"), "step_7"),
        (_("NDA & Employment Agreement Signed"), "step_8"),
        (_("Laptop / Desktop Issued"), "step_9"),
        (_("Email ID Created"), "step_10"),
        (_("System Login Credentials Shared"), "step_11"),
        (_("Access to Tools & Platforms Granted (CRM, Project Tools, Drive, etc.)"), "step_12"),
        (_("Asset Acknowledgment Form Signed"), "step_13"),
        (_("Code of Conduct Signed"), "step_14"),
        (_("Data Protection & Confidentiality Policy Signed"), "step_15"),
        (_("POSH / Workplace Policy Acknowledged"), "step_16"),
        (_("IT & Security Policy Acknowledged"), "step_17"),
    ]
    return render(
        request,
        "onboarding/onboarding_checklist.html",
        {
            "progress_list": progress_list,
            "step_labels": step_labels,
            "filter_archived": request.GET.get("archived", ""),
            "filter_joining_from": request.GET.get("joining_date_from", ""),
            "filter_joining_to": request.GET.get("joining_date_to", ""),
        },
    )
...
@login_required
@require_POST
@permission_required(perm="onboarding.view_onboardingstage")
def onboarding_checklist_step_toggle(request, progress_id):
    """Toggle a checklist step (1-17) for an OnboardingProgress. Returns JSON.
    When unchecking, 'reason' is required in POST."""
    step_map = {
        str(i): f"step_{i}" for i in range(1, 18)
    }
    step_key = request.POST.get("step")
    if step_key not in step_map:
        return JsonResponse({"ok": False, "error": "invalid step"}, status=400)
    progress = get_object_or_404(OnboardingProgress, id=progress_id)
    field = step_map[step_key]
    current = getattr(progress, field)
    new_value = not current
    if not new_value:
        # Unchecking: reason is required
        reason = (request.POST.get("reason") or "").strip()
        if not reason:
            return JsonResponse(
                {"ok": False, "error": "reason_required", "message": _("Reason is required when unchecking.")},
                status=400,
            )
    setattr(progress, field, new_value)
    progress.save()
    # Log the action only if not a duplicate (same user, same outcome, within 2 seconds)
    reason_val = request.POST.get("reason", "").strip() or None
    cutoff = timezone.now() - timedelta(seconds=2)
    duplicate = OnboardingChecklistLog.objects.filter(
        progress=progress,
        step=field,
    ...
    if not duplicate:
        OnboardingChecklistLog.objects.create(
            progress=progress,
            step=field,
            is_checked=new_value,
            user=request.user,
    ...
    return JsonResponse({"ok": True, "done": new_value})


STEP_FIELD_TO_NUM = {f"step_{i}": str(i) for i in range(1, 18)}


@login_required
@require_POST
@permission_required(perm="onboarding.view_onboardingstage")
def onboarding_checklist_upload(request, progress_id):
    """Upload an optional document for a checklist step; stored in Employee Documents."""
    progress = get_object_or_404(OnboardingProgress, id=progress_id)
    step_key = request.POST.get("step")
    step_map = {str(i): f"step_{i}" for i in range(1, 18)}
    if not step_key or step_key not in step_map:
        return JsonResponse({"ok": False, "error": "invalid step"}, status=400)
    field_name = step_map[step_key]
    step_labels = dict(STEP_FIELD_CHOICES)
    title = str(step_labels.get(field_name, field_name))
    ...
    return JsonResponse({"ok": True, "message": _("Document uploaded. It will appear in Employee Documents.")})


@login_required
@require_POST
@permission_required(perm="onboarding.view_onboardingstage")
def onboarding_archive_progress(request, progress_id):
    """Archive an onboarding progress; redirects back to onboarding view with filters."""
    progress = get_object_or_404(OnboardingProgress, id=progress_id)
    progress.is_archived = True
    progress.save()
    base = reverse("onboarding-view")
    from urllib.parse import urlencode
    q = []
    for key in ("archived", "joining_date_from", "joining_date_to", "page"):
...
@login_required
@permission_required(perm="onboarding.view_onboardingstage")
def onboarding_checklist_logs(request, progress_id):
    """GET: return checklist log entries per step for hover display. JSON."""
    from django.utils.timezone import localtime

    progress = get_object_or_404(OnboardingProgress, id=progress_id)
    logs = (
        OnboardingChecklistLog.objects.filter(progress=progress)
        .select_related("user")
        .order_by("step", "-created_at")
    )
    steps = {}
    for log in logs:
        step_num = STEP_FIELD_TO_NUM.get(log.step)
        if step_num not in steps:
            steps[step_num] = []
        if len(steps[step_num]) >= 10:
            continue
        at_display = ""
        if log.created_at:
            local_dt = localtime(log.created_at)
            at_display = local_dt.strftime("%b %d, %Y %H:%M")
        steps[step_num].append(
            {
                "action": "checked" if log.is_checked else "unchecked",
                "user": log.user.get_full_name() or (log.user.get_username() if log.user else ""),
...
def _rejection_email_template_context(candidate):
    """Context dict for candidate/emails/rejection_email.html (sent mail and preview)."""
    company_obj = getattr(getattr(candidate, "recruitment_id", None), "company_id", None)
    company_name = (
        getattr(company_obj, "company", None)
        or getattr(candidate, "get_company", lambda: None)()
        or "Geekonomy"
    )
    job_title = ""
    try:
        job_title = candidate.get_job_position()
    except Exception:
        job_title = (
            getattr(getattr(candidate, "job_position_id", None), "job_position", "")
            or ""
        )
    if not job_title:
        job_title = "Job"
    return {
        "candidate_name": getattr(candidate, "name", None) or str(candidate),
        "job_title": job_title,
...
def preview_rejection_candidate_email(request):
    """
    Full HTML document for the rejection email, for iframe preview in the reject modal.
    (Not an HTMX request — loaded in iframe src.)
    """
    candidate_id = request.GET.get("candidate_id")
    if not candidate_id:
        return HttpResponse(_("Candidate not specified."), status=400)
...
def add_to_rejected_candidates(request):
    ...
    Allowed if user has add_rejectedcandidate or change_candidate (same as Reject button visibility).
    """
    candidate_id = request.GET.get("candidate_id")
    instance = None
    if candidate_id:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponse("<script>window.location.reload()</script>")
    preview_id = candidate_id or (
        request.POST.get("candidate_id") if request.method == "POST" else None
    )
    return render(
        request,
