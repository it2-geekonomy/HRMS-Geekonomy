...
from recruitment.methods import get_recruitment_survey_questions
...
class ApplicationForm(RegistrationForm):
    ...
    def clean_email(self):
        """
        Validate email uniqueness. Check if email already exists before other validations fail.
        """
        ...
        if email:
            ...
            if existing_candidate:
                # Only raise error if this is a new candidate (not updating existing)
                if not self.instance or not self.instance.pk:
                    raise ValidationError(
                        _("A candidate with this email address already exists. Please use a different email or contact support if you believe this is an error.")
                    )
        return email
...
class QuestionForm(ModelForm):
    ...
    class Meta:
        ...
        labels = {
            ...
            "allow_other": _("Allow Other option"),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "allow_other" in fields_order:
            fields_order.remove("allow_other")
            insert_at = 0
            for index, field_name in enumerate(fields_order):
                if field_name.startswith("options"):
                    insert_at = index + 1
            fields_order.insert(insert_at, "allow_other")
        self.fields = {field: self.fields[field] for field in fields_order}
        self.fields["allow_other"].widget.attrs.update(
            {"class": "oh-switch__checkbox"}
        )
        self.last_option_field_name = "options"
        for field_name in self.fields:
            if field_name.startswith("options"):
                self.last_option_field_name = field_name
...
class SurveyForm(forms.Form):
    ...
    def __init__(self, recruitment, *args, **kwargs) -> None:
        ...
        all_questions = get_recruitment_survey_questions(recruitment)
        context = {
            "form": self,
            "questions": all_questions.distinct(),
            "survey_mandatory": recruitment.survey_mandatory,
            "is_preview": False,
        }
...
class SurveyPreviewForm(forms.Form):
    ...
    def __init__(self, template, *args, **kwargs) -> None:
        ...
        all_questions = RecruitmentSurvey.objects.filter(
            template_id=template
        ).order_by("sequence")
        context = {
            "form": self,
            "questions": all_questions.distinct(),
            "survey_mandatory": False,
            "is_preview": True,
        }
        form = render_to_string("survey_form.html", context)
        self.form = form
...
class RejectedCandidateForm(ModelForm):
    """
    RejectedCandidateForm - single text box for "Reject reason" (no dropdown).
    Uses description field; reject_reason_id (dropdown) is excluded.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Reject reason as plain text box (description field)
        self.fields["description"].label = _("Reject reason")
        self.fields["description"].required = False
        self.fields["description"].widget = forms.TextInput(
            attrs={"class": "oh-input w-100", "placeholder": _("Reject reason")}
        )
        self.fields["send_email"] = forms.BooleanField(
            required=False,
            initial=False,
            label=_("Send rejection email to candidate"),
            widget=forms.CheckboxInput(attrs={"class": "oh-switch__checkbox"}),
        )
        # Put send_email just after description for a better UX
        order = list(self.fields.keys())
        if "send_email" in order:
            order.remove("send_email")
        if "description" in order:
            order.insert(order.index("description") + 1, "send_email")
        else:
            order.append("send_email")
        self.fields = {k: self.fields[k] for k in order}

    def save(self, commit=True):
        instance = super().save(commit=commit)
        if commit and instance.pk:
            instance.reject_reason_id.clear()  # use only text reason, not dropdown
        return instance
...
class ScheduleInterviewForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "interview_link" in self.fields:
            self.fields["interview_link"].widget.attrs["placeholder"] = _(
                "e.g. https://meet.google.com/xxx-xxxx-xxx"
            )
        # Insert send_email after description for logical order
        self.fields["send_email"] = forms.BooleanField(
            required=False,
            initial=False,
            label=_("Send email to candidate and interviewers"),
            widget=forms.CheckboxInput(attrs={"class": "oh-switch__checkbox"}),
        )
        order = list(self.fields.keys())
        if "send_email" in order:
