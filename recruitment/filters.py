...
class CandidateFilter(FilterSet):
    ...
    rejected = django_filters.BooleanFilter(
        method="filter_rejected",
        label=_("Rejected"),
        widget=django_filters.widgets.BooleanWidget(),
    )

    def filter_rejected(self, queryset, name, value):
        """Filter candidates that have been added to rejected candidates."""
        if value is True:
            return queryset.filter(rejected_candidate__isnull=False)
        return queryset
    ...
    class Meta:
        ...
        fields = [
            ...
            "rejected_candidate__email_sent",
            "offer_letter_status",
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "rejected_candidate__email_sent" in form_fields:
            form_fields["rejected_candidate__email_sent"].widget.attrs[
                "onchange"
            ] = "$('#statusRejected').val('true'); $('#statusConverted').val('');"
    ...
    def _update_field_labels(self, form_fields):
        ...
        field_label_map = {
            ...
            "rejected_candidate__email_sent": _("Rejection email"),
...
class InterviewFilter(FilterSet):
    """
    Filter set class for Interview schedule.
    Status filter: scheduled (default, today & upcoming), expired (past only), all.
    """
    ...
    status = django_filters.ChoiceFilter(
        choices=[
            ("scheduled", _("Scheduled (today & upcoming)")),
            ("expired", _("Expired")),
            ("all", _("All")),
        ],
        label=_("Status"),
