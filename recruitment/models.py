...
def validate_mobile(value):
    """
    Validate phone/mobile number: only digits, spaces, and optional leading + allowed.
    Rejects letters and other non-numeric characters.
    """
    if value is None or (isinstance(value, str) and not value.strip()):
        return
    value = str(value).strip()
    # Allow digits, spaces, and optional + at the beginning only
    if not re.match(r"^\+?[0-9][0-9\s]*$", value):
        if any(c.isalpha() for c in value):
            raise forms.ValidationError(
                _("Invalid phone number: only digits are allowed. Letters are not allowed.")
            )
        if "+" in value[1:]:
            raise forms.ValidationError(
                _("Invalid phone number: Plus (+) should only appear at the beginning.")
            )
        raise forms.ValidationError(
            _("Invalid phone number: only digits, spaces, and optional + at the start are allowed.")
        )
...
class Recruitment(HorillaModel):
    ...
    survey_mandatory = models.BooleanField(
        default=False,
        help_text=_("Require candidates to complete the survey during application"),
        verbose_name=_("Survey Mandatory"),
    )
    ...
    def total_hires(self):
        """
        Count hired candidates, excluding rejected offers and cancelled rows.
        """
        return (
            self.candidate.filter(hired=True, canceled=False)
...
class Candidate(HorillaModel):
    ...
    def is_offer_rejected(self):
        """
        True if this candidate has been added to rejected candidates (has a RejectedCandidate record).
        Works whether reject reason was saved as text (description) or from dropdown (reject_reason_id).
        """
        return RejectedCandidate.objects.filter(candidate_id=self).exists()
...
class RejectedCandidate(HorillaModel):
    ...
    email_sent = models.BooleanField(
        default=False,
        verbose_name=_("Rejection Email Sent"),
    )
    objects = HorillaCompanyManager(
    ...
    def __str__(self) -> str:
        if self.description:
            return f"{self.candidate_id} - {self.description}"
...
class RecruitmentSurvey(HorillaModel):
    ...
    allow_other = models.BooleanField(
        default=False,
        verbose_name=_("Allow Other option"),
        help_text=_("Adds an Other choice with a text field for applicants."),
    )
    ...
    def choices(self):
        ...
        if not self.options:
            return []
        return [choice.strip() for choice in self.options.split(", ") if choice.strip()]
...
class InterviewSchedule(HorillaModel):
    ...
    interview_link = models.URLField(
        ...
        blank=True,
        help_text=_("e.g. Google Meet, Zoom or other meeting URL"),
    )
