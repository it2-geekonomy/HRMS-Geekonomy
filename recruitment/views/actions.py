...
from recruitment.methods import purge_candidate_protected_relations
...
def _parse_candidate_id_list(ids_raw: str) -> list[int]:
    """JSON array from bulk forms; drop non-numeric entries (e.g. checkbox value \"on\")."""
    ...
    try:
        try:
            with transaction.atomic():
                ...
                purge_candidate_protected_relations(candidate_obj)
...
def candidate_bulk_delete(request):
    ...
    """
    ids = _parse_candidate_id_list(request.POST["ids"])
    for cand_id in ids:
        candidate_obj = None
        try:
            with transaction.atomic():
                candidate_obj = Candidate.objects.get(id=cand_id)
                candidate_label = str(candidate_obj)
                purge_candidate_protected_relations(candidate_obj)
                candidate_obj.delete()
            messages.success(
                request, _("%(candidate)s deleted.") % {"candidate": candidate_label}
            )
        except Candidate.DoesNotExist:
            messages.error(request, _("Candidate not found."))
        except ProtectedError as e:
            name = candidate_obj if candidate_obj is not None else cand_id
            models_verbose_name_set = set()
            for obj in e.protected_objects:
                models_verbose_name_set.add(__(obj._meta.verbose_name.capitalize()))
            models_verbose_name_str = ",".join(models_verbose_name_set)
            messages.error(
                request,
                _("Cannot delete %(candidate)s: still linked to %(models)s.")
                % {"candidate": name, "models": models_verbose_name_str},
            )
...
def candidate_bulk_archive(request):
    ...
    for cand_id in ids:
        candidate_obj = Candidate.objects.get(id=cand_id)
        candidate_obj.archived = archived
        candidate_obj.save()
        messages.success(
            request,
