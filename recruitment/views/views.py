...
from django.http import FileResponse, HttpResponse, HttpResponseRedirect, JsonResponse, QueryDict
...
from base.models import Company, EmailLog, HorillaMailTemplate, JobPosition, clear_messages
...
from recruitment.methods import recruitment_manages, sync_recruitment_survey_links
    ...
    for rec in recruitments:
        ...
        for stage in stages.order_by("sequence"):
            ...
            stage_candidates = CandidateFilter(
                request.GET,
                _pipeline_active_candidates(stage.candidate_set),
...
def _pipeline_query_params(request):
    """
    Pipeline defaults to open recruitments only (closed=false).
    Without this, a bare /pipeline/ refresh shows open and closed together.
    """
...
def _pipeline_filter_dict(query_params):
    """Build pipeline filter chips; hide default open-only closed=false."""
    filter_dict = parse_qs(query_params.urlencode())
    filter_dict = get_key_instances(Recruitment, filter_dict)
    closed_values = filter_dict.get("closed", [])
    if closed_values and str(closed_values[0]).lower() in ("false", ""):
        filter_dict.pop("closed", None)
    return filter_dict


def _pipeline_active_candidates(queryset):
    """Candidates visible in the recruitment pipeline (excludes converted/archived)."""
    return queryset.filter(
        is_active=True,
        archived=False,
        converted=False,
        converted_employee_id__isnull=True,
    )
...
def recruitment_pipeline(request):
    ...
    if "closed" not in request.GET:
        ...
        return redirect(f"{request.path}?{query}" if query else f"{request.path}?closed=false")

    query_params = _pipeline_query_params(request)
...
def _bootstrap_pipeline_cache(request):
    """
    Populate pipeline cache when it is missing (e.g. different Gunicorn worker).
    Uses request.session['pipeline_get'] from the last filter_pipeline, merged with
    request.GET. Safe to call repeatedly.
    """
    sk = request.session.session_key or "nokey"
    key = sk + "pipeline"
    pg = request.session.get("pipeline_get", "")
    if pg and request.GET:
        get_str = pg + "&" + request.GET.urlencode()
    elif pg:
        get_str = pg
    else:
        get_str = request.GET.urlencode()
    get_params = QueryDict(get_str, mutable=True)
    if "closed" not in get_params:
        get_params["closed"] = "false"

    filter_obj = RecruitmentFilter(get_params)
    stage_filter = StageFilter(get_params)
    candidate_filter = CandidateFilter(get_params)
    ...
    filter_dict = _pipeline_filter_dict(get_params)

    CACHE.set(
        key,
        {
            "candidates": _pipeline_active_candidates(candidate_filter.qs).order_by(
                "sequence"
            ),
            "stages": stage_filter.qs.order_by("sequence"),
            "recruitments": recruitments,
            "filter_dict": filter_dict,
            "filter_query": get_params,
        },
    )


def _get_pipeline_cache(request):
    """
    Return the pipeline cache dict. If missing (e.g. another worker),
    bootstrap from session/GET and return. Never returns None.
    """
    sk = request.session.session_key or "nokey"
    key = sk + "pipeline"
    data = CACHE.get(key)
    if data is None:
        _bootstrap_pipeline_cache(request)
        data = CACHE.get(key)
    return data or {
...
def filter_pipeline(request):
    ...
    closed = query_params.get("closed")
    filter_dict = _pipeline_filter_dict(query_params)
    ...
    request.session["pipeline_get"] = query_params.urlencode()

    previous_data = query_params.urlencode()
    paginator = Paginator(recruitments, 3)
...
def get_stage_badge_count(request):
    ...
    count = _pipeline_active_candidates(stage.candidate_set).count()
...
def stage_component(request, view: str = "list"):
    ...
    cache = _get_pipeline_cache(request)
    ordered_stages = cache["stages"].filter(recruitment_id__id=recruitment_id)
...
def update_candidate_stage_and_sequence(request):
    ...
    for index, cand_id in enumerate(order_list):
        candidate = cache["candidates"].filter(id=cand_id)
...
def candidate_component(request):
    ...
    # Do not show rejected candidates in the pipeline (they belong in Rejected Candidates).
    # Candidate.is_offer_rejected is a method, so filter via related RejectedCandidate rows.
    candidates = candidates.exclude(rejected_candidate__isnull=False)

    template = "pipeline/components/candidate_stage_component.html"
    if cache["filter_query"].get("view") == "card":
...
def change_candidate_stage(request):
    ...
    if request.method == "POST":
        ...
        if request.GET.get("bulk") == "True":
            ...
            context["message"] = context.get("message", _("Candidate stage updated successfully."))
        else:
            try:
                ...
                if stage:
                    ...
                    context["message"] = _("Stage not found.")
            except Candidate.DoesNotExist:
                messages.error(request, _("Candidate not found."))
                context["success"] = False
                context["message"] = _("Candidate not found.")
        return JsonResponse(context)
...
@login_required
@hx_request_required
@manager_can_enter(perms=["recruitment.view_stagenote", "recruitment.view_candidate"])
...
def candidate_view(request):
    ...
    previous_data = request.GET.urlencode()
    candidates = Candidate.objects.filter(is_active=True, archived=False)
    # Show rejected candidates only when user explicitly filters by Rejected
    if request.GET.get("rejected") != "true":
        candidates = candidates.filter(rejected_candidate__isnull=True)
    ...
    if Candidate.objects.filter(archived=False).exists():
...
@login_required
@permission_required(perm="recruitment.view_candidate")
def archived_candidates_view(request):
    """
    List archived candidates. From here users can restore candidates to their recruitment.
    """
    previous_data = request.GET.urlencode()
    candidates = Candidate.objects.filter(archived=True).order_by("-id")
    filter_obj = CandidateFilter(request.GET, queryset=candidates)
    recruitments = Recruitment.objects.filter(closed=False, is_active=True)
    return render(
        request,
        "candidate/archived_candidates.html",
        {
            "data": paginator_qry(filter_obj.qs, request.GET.get("page")),
            "pd": previous_data,
            "f": filter_obj,
...
@login_required
@permission_required(perm="recruitment.change_candidate")
def archive_candidate(request, cand_id):
    """
    Archive a candidate. They move to Archived Candidates and are hidden from pipeline/candidate list.
    Accepts both GET and POST so form submissions and direct links work.
    """
    candidate = get_object_or_404(Candidate, id=cand_id)
    if candidate.archived:
        messages.info(request, _("Candidate is already archived."))
    else:
        candidate.archived = True
        candidate.save(update_fields=["archived"])
        messages.success(request, _("Candidate archived."))
    redirect_url = request.META.get("HTTP_REFERER") or reverse("candidate-view")
...
@permission_required(perm="recruitment.change_candidate")
@require_http_methods(["POST"])
def restore_candidate(request, cand_id):
    """
    Restore an archived candidate back to their original recruitment/stage.
    """
    candidate = get_object_or_404(Candidate, id=cand_id, archived=True)
    candidate.archived = False
    candidate.save(update_fields=["archived"])
    messages.success(request, _("Candidate restored to recruitment."))
    return redirect("archived-candidates")
...
def interview_filter_view(request):
    """
    This method is used to filter interviews. By default excludes expired (past) interviews;
    when user applies a filter, expired interviews can be included.
    """
    ...
    # By default exclude expired; show expired only when user filters by status=expired or status=all
    status_param = request.GET.get("status")
    if status_param == "expired":
        interviews = interviews.filter(interview_date__lt=today)
    elif status_param == "all":
        pass  # show all
    else:
        # default or status=scheduled: only today and future
        interviews = interviews.filter(interview_date__gte=today)
...
def interview_view(request):
    """
    This method render interviews to the template. By default shows only scheduled
    (today and future) interviews; expired interviews appear only when a filter is applied.
    """
    previous_data = request.GET.urlencode()
    today = timezone.now().date()
...
def candidate_view_list(request):
    ...
    if request.GET.get("is_active") is None:
        candidates = candidates.filter(is_active=True, archived=False)
    # Show rejected candidates only when user explicitly filters by Rejected
    if request.GET.get("rejected") != "true":
        candidates = candidates.filter(rejected_candidate__isnull=True)
...
@hx_request_required
@permission_required(perm="recruitment.change_candidate")
def candidate_stage_select(request, cand_id):
    """
    This method is used to render the stage selection form for a candidate.
    """
    candidate = Candidate.find(cand_id)
    if not candidate:
        return render(request, "candidate/stage_select.html", {"error": _("Candidate not found")})

    stages = Stage.objects.filter(recruitment_id=candidate.recruitment_id)
    return render(request, "candidate/stage_select.html", {"candidate": candidate, "stages": stages})


@login_required
@hx_request_required
@permission_required(perm="recruitment.change_candidate")
def candidate_stage_change_modal(request):
    """
    This method is used to update candidate stage from modal and return HTML response
    """
    if request.method == "POST":
        ...
        try:
            ...
            if stage:
                ...
                success = True
                message = message or _("Candidate stage updated successfully.")
                messages.success(request, _("Candidate stage updated"))
            else:
                success = False
                message = _("Stage not found.")
        except Candidate.DoesNotExist:
            success = False
            message = _("Candidate not found.")

        return render(request, "candidate/stage_select.html", {"success": success, "message": message})
...
def candidate_resume_file(request, cand_id):
    """
    Serve the candidate's resume file through Django. Uses storage backend to handle
    both local and remote storage (S3, etc.). Handles backward compatibility for old
    resume paths. Returns friendly message if file missing.
    """
    ...
    if not candidate_obj or not candidate_obj.resume:
        html = (
            "<!DOCTYPE html><html><body style='font-family:sans-serif;padding:2rem;text-align:center;'>"
            "<p style='color:#666;'>Resume not on file for this candidate.</p></body></html>"
        )
    ...
    if not file_name:
        html = (
            "<!DOCTYPE html><html><body style='font-family:sans-serif;padding:2rem;text-align:center;'>"
            "<p style='color:#666;'>Resume file path is not available.</p></body></html>"
        )
        return HttpResponse(html, content_type="text/html; charset=utf-8")
    
    # Try multiple possible paths for backward compatibility
    possible_paths = [file_name]  # Start with the stored path
    
    # Extract filename for alternative path construction
    filename_only = os.path.basename(file_name)
    name_slug = slugify(candidate_obj.name) if candidate_obj.name else "candidate"
    
    # New format: recruitment/candidate/resume/{filename}.pdf
    # Old format: recruitment/{name_slug}/{filename}.pdf
    # Edge case: just filename without directory (e.g., "Zeven (3).pdf")
    
    # Check if path is just a filename (no directory structure)
    is_just_filename = "/" not in file_name
    
    if is_just_filename:
        # If stored path is just filename, try all possible locations
        possible_paths.extend([
            f"recruitment/{name_slug}/{file_name}",  # Old format
            f"recruitment/candidate/resume/{file_name}",  # New format
            f"recruitment/candidate/{file_name}",  # Missing resume subdir
            f"media/{file_name}",  # Root media directory
            f"uploads/{file_name}",  # Uploads directory
        ])
    else:
        # Path has directory structure
        # If path doesn't match new format, try old format
        if not file_name.startswith("recruitment/candidate/resume/"):
            # Try old format path (recruitment/{name_slug}/{filename})
            old_path = f"recruitment/{name_slug}/{filename_only}"
            if old_path not in possible_paths:
                possible_paths.append(old_path)
            
            # Try with just the filename (in case it's stored at root)
            if filename_only not in possible_paths:
                possible_paths.append(filename_only)
            
            # Try recruitment/candidate/{filename} (missing resume subdirectory)
            alt_path = f"recruitment/candidate/{filename_only}"
            if alt_path not in possible_paths:
                possible_paths.append(alt_path)
            
            # If current path has parts, try reconstructing with old format
            if "/" in file_name:
                parts = file_name.split("/")
                # Try different combinations
                if len(parts) >= 2:
                    # Try recruitment/{name_slug}/{last_part}
                    if parts[-1] != filename_only:
                        alt_path2 = f"recruitment/{name_slug}/{parts[-1]}"
                        if alt_path2 not in possible_paths:
                            possible_paths.append(alt_path2)
        
        # Also try the new format even if current path is old format (in case file was moved)
        new_format_path = f"recruitment/candidate/resume/{filename_only}"
        if new_format_path not in possible_paths:
            possible_paths.append(new_format_path)
    ...
    # If storage backend check failed, try direct file system check (for local storage)
    if not file_handle:
        try:
            ...
            # Try alternative paths on file system
            for path in possible_paths:
                try:
    ...
    if not file_handle:
        html = (
            "<!DOCTYPE html><html><body style='font-family:sans-serif;padding:2rem;text-align:center;'>"
            "<p style='color:#666;'>Resume file is not available (file missing on server).</p>"
            "<p style='color:#999;font-size:0.9em;margin-top:1rem;'>"
            f"Tried paths: {', '.join(possible_paths[:3])}</p></body></html>"
        )
        return HttpResponse(html, content_type="text/html; charset=utf-8")

    # Get file extension for content type
    file_ext = os.path.splitext(actual_path or file_name)[1][1:].lower() if (actual_path or file_name) else "pdf"
    content_type = get_content_type(file_ext) or "application/pdf"
    
    # Return the file (as attachment for download, inline for viewer)
    as_attachment = request.GET.get("download") == "1"
    try:
        response = FileResponse(file_handle, content_type=content_type, as_attachment=as_attachment)
        if as_attachment:
            download_name = f"{name_slug}_resume.{file_ext}" if file_ext else f"{name_slug}_resume.pdf"
        ...
        html = (
            "<!DOCTYPE html><html><body style='font-family:sans-serif;padding:2rem;text-align:center;'>"
            f"<p style='color:#666;'>Resume file is not available (error: {str(e)}).</p></body></html>"
        )
        return HttpResponse(html, content_type="text/html; charset=utf-8")
...
@login_required
@manager_can_enter(perm="recruitment.view_candidate")
def candidate_resume_viewer(request, cand_id):
    """
    Serve an HTML viewer for the candidate's resume so that links in the PDF
    open in a new tab (externalLinkTarget). Uses PDF.js in the template.
    """
    candidate_obj = Candidate.find(cand_id)
    if not candidate_obj or not candidate_obj.resume:
        html = (
        ...
        return HttpResponse(html, content_type="text/html; charset=utf-8")
    pdf_url = reverse("candidate-resume-file", args=[cand_id])
    return render(
        request,
        "candidate/resume_viewer.html",
        {"pdf_url": pdf_url},
    )
...
@login_required
@manager_can_enter(perm="recruitment.change_candidate")
def candidate_revert_conversion(request, cand_id, **kwargs):
    """
    Revert a candidate conversion - allow changing status back
    """
    candidate_obj = Candidate.find(cand_id)
    ...
    if not candidate_obj.converted_employee_id:
        messages.info(request, "This candidate is not converted.")
        return HttpResponseRedirect(request.META.get("HTTP_REFERER", "/"))
    
    # Revert conversion
    candidate_obj.converted_employee_id = None
    candidate_obj.converted = False
    candidate_obj.hired = False
    candidate_obj.save()
    
    messages.success(request, _("Candidate conversion reverted successfully."))
...
def candidate_conversion(request, cand_id, **kwargs):
    ...
    if user_exists:
        ...
        try:
            ...
            candidate_obj.converted = True
            candidate_obj.save()
            if OnboardingProgress is not None:
                OnboardingProgress.objects.get_or_create(employee_id=new_employee)
            messages.success(
                request,
...
def _send_interview_schedule_email(interview, request):
    """Send interview details to candidate and all interviewers (professional: logo, contact, reply-to, meet button)."""
    candidate = interview.candidate_id
    to_emails = []
    for emp in interview.employee_id.all():
        e = None
        if getattr(emp, "employee_work_info", None) and emp.employee_work_info:
            e = getattr(emp.employee_work_info, "email", None) or None
        if not e:
            e = getattr(emp, "email", None)
        if e:
            to_emails.append(e)
    if candidate.email:
    ...
    # Use HR email as sender and reply-to (not system default)
    hr_email = getattr(settings, "HR_EMAIL", None) or "hr@thegeekonomy.com"
    hr_phone = getattr(settings, "HR_PHONE", None) or ""
    from_email = f"HR Team <{hr_email}>"

    d = interview.interview_date
    t = interview.interview_time
    time_str = t.strftime("%I:%M %p") if t else ""
    date_str = d.strftime("%B %d, %Y") if d else str(d) if d else ""

    link = (getattr(interview, "interview_link", None) or "").strip()
    desc = (interview.description or "").strip()
    job_pos = ""
    if getattr(candidate, "job_position_id", None) and candidate.job_position_id:
        job_pos = getattr(candidate.job_position_id, "job_position", None) or str(candidate.job_position_id)
    recr = ""
    if getattr(candidate, "recruitment_id", None) and candidate.recruitment_id:
        recr = getattr(candidate.recruitment_id, "title", None) or str(candidate.recruitment_id)
    company = getattr(candidate, "get_company", lambda: None)() or ""
    company_obj = getattr(getattr(candidate, "recruitment_id", None), "company_id", None)
    logo_url = None
    if company_obj and getattr(company_obj, "icon", None) and company_obj.icon:
        try:
            logo_url = request.build_absolute_uri(company_obj.icon.url)
        except Exception:
            pass

    interviewers = ", ".join(emp.get_full_name() for emp in interview.employee_id.all())
    interviewer_contacts = []
    for emp in interview.employee_id.all():
        ph = getattr(emp, "phone", None)
        if not ph or not str(ph).strip():
            wi = getattr(emp, "employee_work_info", None)
            ph = getattr(wi, "mobile", None) if wi else None
        if ph and str(ph).strip():
            interviewer_contacts.append((emp.get_full_name(), str(ph).strip()))

    # Build ICS calendar attachment (add to calendar)
    tz = timezone.get_default_timezone()
    local_dt = timezone.make_aware(datetime.combine(d, t), tz)
    utc_dt = local_dt.astimezone(timezone.utc)
    dtstart_str = utc_dt.strftime("%Y%m%dT%H%M%SZ")
    dtend_dt = utc_dt + timedelta(minutes=45)
    dtend_str = dtend_dt.strftime("%Y%m%dT%H%M%SZ")
    dtstamp_str = timezone.now().strftime("%Y%m%dT%H%M%SZ")
    uid = f"interview-{interview.id}-{candidate.id}@horilla"
    _esc = lambda s: (s or "").replace("\\", "\\\\").replace(";", "\\;").replace(",", "\\,")
    summary_ical = _esc(f"Interview: {candidate.name} - {job_pos or 'Position'}")[:200]
    desc_ical = _esc(f"Interview at {company or 'Company'}. {'Meeting: ' + link if link else ''}")[:500]
    vevent = [f"UID:{uid}", f"DTSTAMP:{dtstamp_str}", f"DTSTART:{dtstart_str}", f"DTEND:{dtend_str}", f"SUMMARY:{summary_ical}", f"DESCRIPTION:{desc_ical}"]
    if link:
        vevent.append(f"URL:{link}")
    ics_lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Horilla HRMS//Interview//EN", "CALSCALE:GREGORIAN", "BEGIN:VEVENT"] + vevent + ["END:VEVENT", "END:VCALENDAR"]
    ics_content = "\r\n".join(ics_lines)

    subject = _("Interview Scheduled: %(candidate)s on %(date)s") % {
    ...
    if logo_url:
        body += f'<p style="margin: 0 0 16px 0;"><img src="{escape(logo_url)}" alt="" style="height: 36px; max-width: 140px; object-fit: contain;" /></p>'
    body += "<p>" + (_("Hello %(name)s,") % {"name": candidate.name}) + "</p>"
    body += f"<p>{_('Your interview has been scheduled. Please find the details below.')}</p>"
    body += f'<p><b>{_("Interview details")}</b></p>'
    body += '<table style="border-collapse: collapse; width: 100%; margin: 8px 0;">'

    def _row(label, value, html=False):
        if value is None or (isinstance(value, str) and value.strip() == ""):
            return ""
        v = value if html else str(value).strip()
        safe = v if html else escape(v)
        return (
            f'<tr><td style="padding: 6px 8px 6px 0; vertical-align: top; width: 140px;"><strong>{label}</strong></td>'
            f'<td style="padding: 6px 0; border-bottom: 1px solid #eee;">{safe}</td></tr>'
        )

    body += _row(_("Candidate"), candidate.name)
    if job_pos:
        body += _row(_("Job position"), job_pos)
    if recr:
        body += _row(_("Recruitment"), recr)
    if company:
        body += _row(_("Company"), company)
    body += _row(_("Interviewer(s)"), interviewers if interviewers else None)
    body += _row(_("Date"), date_str)
    body += _row(_("Time"), time_str)
    if link:
        body += _row(
            _("Meeting link"),
            (
                f'<a href="{escape(link)}" style="display: inline-block; background: #1a73e8; color: #ffffff; '
                f'padding: 10px 20px; text-decoration: none; border-radius: 4px; font-weight: 500;">'
                f'{_("Join video meeting")}</a>'
                f'<br><span style="font-size: 12px; color: #5f6368;">{escape(link)}</span>'
            ),
            html=True,
        )
    if desc:
        body += _row(_("Description"), desc)

    body += "</table>"

    body += f"<p style='margin-top: 12px;'>{_('Please ensure you join the meeting on time and have a stable internet connection.')}</p>"
    body += f"<p>{_('If you are unable to attend at the scheduled time, kindly inform us in advance.')}</p>"
    body += f"<p>{_('For any queries, feel free to contact us.')}</p>"
    body += f"<p style='margin-top: 8px;'>📅 {_('A calendar invite (.ics) is attached — add it to your calendar.')}</p>"
    # Footer (trust factor): Best regards, HR Team, Company, Email, Phone
    body += "<p>"
    body += f"{_('Best regards,')}<br>"
    body += f"{_('HR Team')}<br>"
    if company:
        body += f"{escape(company)}<br>"
    body += f"<strong>{_('Email')}:</strong> {escape(hr_email)}<br>"
    body += f"<strong>{_('Phone')}:</strong> {escape(hr_phone) if hr_phone else '-'}"
    body += "</p>"
    body += "</div>"

    msg = EmailMessage(subject=subject, body=body, to=to_emails, from_email=from_email, reply_to=[hr_email])
    msg.attach("interview.ics", ics_content.encode("utf-8"), "text/calendar")
    msg.content_subtype = "html"
    try:
        msg.send()
    except Exception as e:
        logging.getLogger(__name__).exception(e)
        messages.error(request, _("Failed to send interview email."))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if form.cleaned_data.get("send_email"):
                _send_interview_schedule_email(instance, request)
...
def normalize_skill_name(skill_name):
    """
    Normalize skill names to handle variations like React.js, ReactJS, React, etc.
    Args:
        skill_name: The skill name to normalize
    Returns:
        List of possible variations of the skill name
    """
    skill_lower = skill_name.lower().strip()
    
    # Define skill variations mapping
    skill_variations = {
        'react': ['react', 'reactjs', 'react.js', 'reactjs', 'react native', 'reactjs'],
    ...
    # Check if the skill matches any known variations
    for base_skill, variations in skill_variations.items():
        if any(var in skill_lower for var in variations):
            return variations
    
    # If no variations found, return the original skill and some common variations
    return [skill_lower, skill_lower.replace('.', ''), skill_lower.replace('js', 'javascript')]


def calculate_skill_match_score(required_skills, resume_words):
    """
    Calculate matching score with improved skill matching logic
    Args:
        required_skills: List of required skills from recruitment
        resume_words: List of words extracted from resume
    Returns:
        Number of matching skills found
    """
    matching_count = 0
    resume_text = ' '.join(resume_words)
    
    for skill in required_skills:
        skill_variations = normalize_skill_name(skill)
        
        # Check if any variation of the skill is found in resume
...
def matching_resumes(request, rec_id):
    ...
    for resume in resumes:
        ...
        matching_skills_count = calculate_skill_match_score(skills, words)
...
def hired_candidate_chart(request):
    ...
    for recruitment in recruitments:
        ...
        data.append(
            recruitment.candidate.filter(hired=True, canceled=False)
            .filter(rejected_candidate__isnull=True)
            .count()
        )
    return JsonResponse(
        {
