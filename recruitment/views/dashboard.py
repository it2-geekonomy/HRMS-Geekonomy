...
def dashboard(request):
    ...
    ongoing_recruitment_details = []  # List of {title, managers} for popup

    for rec in recruitment_obj:
        recruitment_title = rec.title or str(rec)
        managers = [m.get_full_name() for m in rec.recruitment_managers.all()]

        recruitment_manager_mapping[recruitment_title] = managers
        ongoing_recruitment_details.append({
            "title": recruitment_title,
            "managers": managers,
        })

    total_vacancy = 0
    vacancy_details = []  # List of (title, vacancy) for popup
    for openings in recruitment_obj:
        if openings.vacancy is not None and openings.vacancy > 0:
            total_vacancy += openings.vacancy
            title = (
                str(openings.job_position_id.job_position)
                if openings.job_position_id and openings.title is None
                else (openings.title or str(openings))
            )
            vacancy_details.append({"title": title, "vacancy": openings.vacancy})
    ...
    hired_candidates_list = [
        {
            "name": c.name or c.email or "—",
            "job_position": str(c.job_position_id.job_position)
            if c.job_position_id and c.job_position_id.job_position
            else "—",
        }
    ...
    return render(
        ...
        "dashboard/dashboard.html",
        {
            ...
            "ongoing_recruitment_details": ongoing_recruitment_details,
            "hired_candidates_list": hired_candidates_list,
