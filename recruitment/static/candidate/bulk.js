...
$("#bulkConvertToEmployee").click(function (e) {
  e.preventDefault();
  var ids = JSON.parse($("#selectedInstances").attr("data-ids") || "[]");
  if (ids.length === 0) {
  ...
  if (ids.length > 1) {
    Swal.fire({ text: "Please select one candidate to convert.", icon: "info", confirmButtonText: "OK" });
    return;
  }
  window.location.href = "/recruitment/candidate-conversion/" + ids[0] + "/";
});

$("#bulkRejected").click(function (e) {
  e.preventDefault();
  var ids = JSON.parse($("#selectedInstances").attr("data-ids") || "[]");
  if (ids.length === 0) {
    Swal.fire({ text: noRowMessages["en"], icon: "warning", confirmButtonText: "Close" });
...
$("#bulkArchiveCandidate").click(function (e) {
  e.preventDefault();
  var languageCode = null;
  getCurrentLanguageCode(function (code) {
    languageCode = code;
