...
class Employee(models.Model):
    ...
    emergency_contact = models.CharField(
        max_length=15, null=True, blank=True, verbose_name=_("Emergency contact number")
    )
    ...
    def get_operation_manager(self):
        """
        This method is used to return the operation manager of the employee
        """
        return getattr(
            getattr(self, "employee_work_info", None), "operation_manager_id", None
        )

    def get_business_manager(self):
        """
        This method is used to return the business manager of the employee
        """
        return getattr(
            getattr(self, "employee_work_info", None), "business_manager_id", None
        )
    ...
    def check_online(self):
        ...
        # Fallback: attendance-based (not clocked out today)
        if apps.is_installed("attendance"):
    ...
    def save(self, *args, **kwargs):
        ...
        prev_email = prev_employee.email if prev_employee else None
        ...
        if employee.employee_user_id is None:
            ...
            username = self.email
            # Use phone as password - strip any formatting and ensure it's valid
            phone_str = str(self.phone).strip() if self.phone else ""
            # Remove common phone formatting (spaces, dashes, parentheses)
            phone_clean = phone_str.replace(" ", "").replace("-", "").replace("(", "").replace(")", "").replace("+", "")
            
            # Use phone as password if valid (at least 6 digits), otherwise use default
            if phone_clean and len(phone_clean) >= 6 and phone_clean.isdigit():
                password = phone_clean
            else:
                # Use email-based default password if phone is invalid
                password = f"{username.split('@')[0]}123" if '@' in username else "123456"

            # Check if username already exists
            if User.objects.filter(username=username).exists():
                # Username exists, try to find existing user or create with unique username
                existing_user = User.objects.filter(username=username).first()
                # Check if user is already linked to another employee
                try:
                    existing_emp = existing_user.employee_get
                    # User is linked to another employee, create with unique username
                    counter = 1
                    while User.objects.filter(username=f"{username}_{counter}").exists():
                        counter += 1
                    username = f"{username}_{counter}"
                    is_new_employee_flag = True
                    try:
                        ...
                        # Fallback if is_new_employee field doesn't exist
                        user = User.objects.create_user(
                    ...
                    # User exists but not linked to employee, use it
                    # Don't change password if user already has one set
                    if not existing_user.has_usable_password() or existing_user.check_password("") or existing_user.check_password("123456"):
                        existing_user.set_password(password)
                        existing_user.save()
                    self.employee_user_id = existing_user
                    # Save employee_user_id without triggering save again
                    Employee.objects.filter(id=self.id).update(employee_user_id=existing_user)
            else:
...
class SlackPresence(models.Model):
    """
    Stores Slack user presence (active/away) from Slack Events API presence_change.
    Maps to Employee via Employee.slack_user_id.
    """
    ...
    class Meta:
        verbose_name = _("Slack Presence")
        verbose_name_plural = _("Slack Presences")
...
class EmployeeWorkInformation(models.Model):
    ...
    operation_manager_id = models.ForeignKey(
        Employee,
        on_delete=models.PROTECT,
        blank=True,
        null=True,
        related_name="operation_manager",
        verbose_name=_("Operation Manager"),
    )
    business_manager_id = models.ForeignKey(
        Employee,
        on_delete=models.PROTECT,
        blank=True,
        null=True,
        related_name="business_manager",
        verbose_name=_("Business Manager"),
    )
    ...
    team_ids = models.ManyToManyField(
        'Team',
        blank=True,
        related_name='work_team_members',
    ...
    PROBATION_ACTION_CHOICES = [
        ("extended", _("Extended")),
        ("confirmed", _("Confirmed")),
        ("rejected", _("Rejected")),
    ]
    probation_action = models.CharField(
        max_length=20,
        blank=True,
        null=True,
        choices=PROBATION_ACTION_CHOICES,
        verbose_name=_("Probation Action"),
    )
...
class Team(HorillaModel):
    ...
    team_name = models.CharField(
        ...
        help_text=_("Name of the team within the department")
    )
    description = models.TextField(
        blank=True,
        null=True,
        verbose_name=_("Description"),
        help_text=_("Brief description of the team's purpose or function")
    )
    ...
    def __str__(self) -> str:
        return f"{self.team_name} - {self.department_id}"
