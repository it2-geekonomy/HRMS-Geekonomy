...
from django.db.models.functions import Cast, Coalesce, NullIf, Substr
...
# Emp Id format: GEEKY + exactly 4 digits (e.g. GEEKY0001)
EMP_ID_PREFIX = "GEEKY"
...
def get_next_emp_id():
    """
    Return next Emp Id: GEEKY + 4 digits. Finds max existing GEEKYxxxx and increments.
    """
    existing = (
    ...
    max_num = 0
    for bid in existing:
        if isinstance(bid, str) and EMP_ID_PATTERN.match(bid.strip()):
            try:
                num = int(bid.strip()[-4:])
                max_num = max(max_num, num)
            except ValueError:
                pass
    return f"{EMP_ID_PREFIX}{max_num + 1:04d}"
...
def order_employees_by_badge_id_numeric(queryset):
    """
    Order employee queryset by badge_id numerically (GEEKY0001, GEEKY0002, ...).
    Uses Substr(badge_id, 6) to extract the 4 digits after 'GEEKY'. NULL/invalid
    badge_ids are ordered last.
    """
    return queryset.annotate(
        _badge_num=Coalesce(
            Cast(NullIf(Substr("badge_id", 6), Value("")), IntegerField()),
            Value(999999),
        )
    ).order_by("_badge_num")


def order_employees_by_joining_date(queryset):
    """
    Order employee queryset by joining date (oldest first).
    Employees without date_joining are ordered last.
    """
    return queryset.order_by(
        F("employee_work_info__date_joining").asc(nulls_last=True),
        "id",
    )
